...
void CalcEmitter(SND_EMITTER* emitter, f32* vol, f32* doppler, f32* xPan, f32* yPan, f32* zPan);
int AddStartingEmitter(SND_EMITTER* emitter, f32 vol, f32 xPan, f32 yPan, f32 zPan, f32 pitch);
void AddRunningEmitter(SND_EMITTER* emitter, f32 vol);
void SetFXParameters(SND_EMITTER* emitter, f32 vol, f32 xPan, f32 yPan, f32 zPan, f32 pitch);
...
void s3dHandle(void) {
    ...
    for (; em != (SND_EMITTER*)0x0; em = next) {
        ...
        if ((flags & S3D_EMITTER_FLAG_WAITING_FOR_ROOM) == 0) {
            if ((flags & S3D_EMITTER_FLAG_PLAYING) != 0) {
                if ((0.0f == vol) && ((flags & S3D_EMITTER_FLAG_STOP_AT_ORIGIN) != 0)) {
                    ...
                    if (((entry == (SND_ROOM*)0x0) || (entry->studio != 0xff)) &&
                        (em->vid = synthFXStart(em->fxid, S3D_DEFAULT_FX_VOLUME, S3D_DEFAULT_FX_PAN,
            ...
            entry = em->room;
            if (((entry == (SND_ROOM*)0x0) || ((entry != (SND_ROOM*)0x0) && (entry->studio != 0xff))) &&
                (0.0f != vol)) {
                em->flags &= ~S3D_EMITTER_FLAG_WAITING_FOR_ROOM;
                em->flags |= S3D_EMITTER_FLAG_PLAYING;
            }
