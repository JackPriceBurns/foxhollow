...
void mcmdPortamento(McmdVoiceState* state, McmdCommandArgs* args) {
    ...
    case 1:
        ...
        while (TRUE) {
            ...
            break;
        case 2:
            if (state->midi != 0xff && inpGetMidiCtrl(MCMD_CTRL_PORTAMENTO, state->midi, state->midiSet) > 0x1f80) {
                continue;
            }
            break;
        }
...
static inline u32 macPostMessage(u32 vid, u32 mesg) {
    ...
    if ((v = vidGetInternalId(vid)) != 0xffffffff && (sv = &synthVoice[v & 0xff])->mesgNum < 4) {
        ++sv->mesgNum;
    ...
    if (((args->flags >> 8) & 0xff) == 0) {
        ...
        if (targetInstrument != 0xffff) {
            for (i = 0; i < SYNTH_CONFIGURATION->voiceCount; i++) {
                if (synthVoice[i].addr != 0 && targetInstrument == synthVoice[i].macroId) {
    ...
    do {
        ...
        *cmdValuePtr = musyxReadBE32(&((McmdCommandArgs*)sv->curAddr)->value);
        ...
        case 0x16: /* set ADSR from ctrl */
        {
            ...
            row = (s32*)(dataTables +
                         (inpGetMidiCtrl((macCurrentCmd.flags >> 0x10) & 0xff, sv->midi, sv->midiSet) >> 7) * 4);
