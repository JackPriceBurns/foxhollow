...
TrackTriangle* trackBuildBlockTriangles(TrackTriangle* cur, int x0, int y0, int z0, int x1, int y1, int z1, int flags, u8 doEdges);
TrackTriangle* trackBuildModelTriangles(TrackTriangle* cur, TrackBlockDescriptor* desc, ModelFileHeader** model, f32 scale, f32 x0, f32 y0, f32 z0,
...
int trackSweepSphereAgainstEdge(const f32* edgeStart, const f32* edgeDirection, f32 edgeLength,
                                f32 radius, f32 radiusSq,
                                f32* rayOrig, f32* rayDir, f32 maxd, f32* out29, f32* outNrm,
    ...
    if (gTrackSweepHitCount != 0 && out != NULL)
    {
        ...
        if (parentAddress != NULL)
        {
            ...
            Obj_TransformWorldPointToLocal(out->lineEndX, out->lineEndY, out->lineEndZ, &out->lineEndX,
                                           &out->lineEndY, &out->lineEndZ, parentAddress);
        }
                ...
                if (mag != 0.0f)
                {
                         ...
                         tri < gTrackTriangleBuffer + desc[1].firstTriangle; tri++)
                    {
                        ...
                        for (edgeBit = 0; edgeBit < 3; edgeBit++)
                        {
                            ...
                            if (trackSweepSphereAgainstEdge(va, evec, rdata[2], rdata[0], rdata[1], ws, dir, mag,
                                                            hitpt, plane, maxStep, &frac, 0.0f))
                            {
                                hit = 1;
                                goto hitCheck;
                            }
