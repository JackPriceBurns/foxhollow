    ...
    for (; i < count; i++)
    {
        ...
        for (; tri < triEnd; tri += 0x14)
        {
            ...
            for (; t0 < vEnd; t0++, vq += 8)
            {
                ...
                cur->vy[0] = minY;
                cur->vz[0] = minZ + dzoff;
                v0[0] = __OSs16tof32(&cur->vx[0]);
                v0[1] = __OSs16tof32(&cur->vy[0]);
                v0[2] = __OSs16tof32(&cur->vz[0]);
                maxYi = 0;
                minYi = 0;
                j = 1;
                ...
                PSVECCrossProduct((Vec*)e0, (Vec*)e1, (Vec*)cur->planeN);
                ...
                cur->planeD = -PSVECDotProduct((Vec*)cur->planeN, (Vec*)v0);
                if (doEdges)
                {
                    int k22, deg, j2;
                    f32* ep;
