...
typedef struct IntersectLine IntersectLine;

TrackTriangle* gTrackTriangleBufferEnd;
...
/* When flag 0x80 is set, the two endpoint bytes are one signed big-endian
 * height. The original PowerPC code could read them through an s16 pointer;
...
TrackTriangle* trackBuildBlockTriangles(TrackTriangle* cur, int x0, int y0, int z0, int x1, int y1, int z1, int flags,
                                        u8 doEdges);
TrackTriangle* trackBuildModelTriangles(TrackTriangle* cur, TrackBlockDescriptor* desc, ModelFileHeader** model,
                                        f32 scale, f32 x0, f32 y0, f32 z0, f32 x1, f32 y1, f32 z1, u8 flags);
...
int trackSweepSphereAgainstEdge(const f32* edgeStart, const f32* edgeDirection, f32 edgeLength, f32 radius,
                                f32 radiusSq, f32* rayOrig, f32* rayDir, f32 maxd, f32* out29, f32* outNrm, f32 maxStep,
...
int trackSweepCircleAgainstLines(f32* startPos, f32* endPos, f32 radius, int flags, TrackBBoxHit* hit,
    ...
    if (target != NULL) {
        ...
        lineIdx = NULL;
        vt = target->anim.modelInstance->intersectionLines;
        vp = (u8*)target->anim.modelInstance->intersectionPoints;
        ...
        vt = (u8*)gIntersectLinePool;
        vp = (u8*)gIntersectPoints;
    }
    ...
    while (found) {
        ...
        for (i = start; i < end; i++) {
            ...
            if (lineFlags & 0x80) {
                ha = (f32)trackReadSharedLineHeight(rec);
                hb = ha;
            } else {
                ha = (f32)(s8)rec[0];
                hb = (f32)(s8)rec[1];
    ...
    if (count != 0 && hit != NULL) {
        ...
        }
        {
            ...
            if ((s8) * (u8*)((u8*)rec2 + 2) & 0x80) {
                fa = trackReadSharedLineHeight(rec2);
                fb = fa;
            } else {
                fa = (f32)(s8) * (u8*)rec2;
                fb = (f32)(s8) * ((u8*)rec2 + 1);
...
void intersectModLineBuild(ObjDef* obj) {
    ...
    for (outputLineIndex = 0; outputLineIndex < gIntersectLineCount; outputLineIndex++) {
        ...
        memcpy(&((IntersectLine*)obj->intersectionLines)[outputLineIndex], (char*)gIntersectLinePool + best * 0x10,
               0x10);
...
void trackIntersect(void) {
    ...
    if (lbl_803DCF40 != 0) {
        ...
        while (sortComplete == 0) {
            ...
            for (sortIndex = 0, lineOffset = sortIndex; sortIndex < gIntersectLineCount - 1;
                ...
                sortOrder = (s16*)((u8*)lbl_803DCF40 + lineOffset);
    ...
    for (i = 0, lineOffset = i; i < gIntersectLineCount; lineOffset += 2, i++) {
        segmentType = (s16)(gIntersectLinePool[gIntersectLineIndexTable[i]].kind & 0x3f);
        if (segmentType >= 0x14) {
            segmentType = 1;
            debugPrintf(sTrackIntersectFuncOverflowFormat, 1);
        }
...
int trackSweepSphereAgainstEdge(const f32* edgeStart, const f32* edgeDirection, f32 edgeLength, f32 radius,
    ...
    Vec3_Cross(rayDir, edgeDirection, nrm);
    len = Vec3_Normalize(nrm);
    if (0.0f == len) {
        return 0;
    }
    e[0] = rayOrig[0] - edgeStart[0];
    e[1] = rayOrig[1] - edgeStart[1];
    e[2] = rayOrig[2] - edgeStart[2];
    {
        f32 d0 = nrm[1] * e[1];
    ...
    f29 = f29 * f29;
    if (f29 <= radiusSq) {
        Vec3_Cross(e, edgeDirection, tmp14);
        {
            f32 dl = tmp14[1] * nrm[1];
            len = -(dl + tmp14[0] * nrm[0] + tmp14[2] * nrm[2]) / len;
        }
        Vec3_Cross(nrm, edgeDirection, tmp14);
        Vec3_Normalize(tmp14);
        {
            f32 s = sqrtf(radiusSq - f29);
        ...
        if (len >= zero) {
            if (len <= maxd) {
                ...
                {
                    f32 d2 = edgeDirection[1] * edgeStart[1];
                    f12 = (hit[0] * edgeDirection[0] + hit[1] * edgeDirection[1] + hit[2] * edgeDirection[2]) -
                          (d2 + edgeDirection[0] * edgeStart[0] + edgeDirection[2] * edgeStart[2]);
                }
                if (f12 >= zero) {
                    if (f12 <= edgeLength) {
                        tmp14[0] = edgeDirection[0] * f12;
                        tmp14[1] = edgeDirection[1] * f12;
                        tmp14[2] = edgeDirection[2] * f12;
                        tmp14[0] = edgeStart[0] + tmp14[0];
                        tmp14[1] = edgeStart[1] + tmp14[1];
                        tmp14[2] = edgeStart[2] + tmp14[2];
                        ...
                        Vec3_Normalize(outNrm);
                        {
                            ...
                            outNrm[3] = radius - (dh + hit[0] * outNrm[0] + hit[2] * outNrm[2]);
                        }
        ...
        do {
                ...
                if (mag != 0.0f) {
                    for (tri = gTrackTriangleBuffer + desc->firstTriangle;
                        ...
                        for (edgeBit = 0; edgeBit < 3; edgeBit++) {
                            ...
                            if (trackSweepSphereAgainstEdge(va, evec, rdata[2], rdata[0], rdata[1], ws, dir, mag, hitpt,
                                                            plane, maxStep, &frac, 0.0f)) {
                                hit = 1;
                                goto hitCheck;
                            }
                        }
            ...
            hitCheck:
                if (hit != 0) {
                    ...
                    found = 1;
                    if ((u8)type == 7) {
                        results->planes[i][0] = norm4[0];
                        results->planes[i][1] = norm4[1];
                        results->planes[i][2] = norm4[2];
                        results->planes[i][3] = norm4[3];
                        results->surfaceTypes[i] = typeb;
                        results->triangleFlags[i] = triFlags;
                        results->objects[i] = objmtx;
                        bounces++;
                        goto slotComplete;
                    }
                    break;
            ...
            if (found != 0) {
                ...
                if (bounces > 10) {
                    ...
                    results->planes[i][3] = norm4[3];
                    results->surfaceTypes[i] = typeb;
                    results->triangleFlags[i] = typeb2;
                    results->objects[i] = objmtx;
                }
            }
        } while (found != 0);
...
int trackGetIntersect(GameObject* contactSrc, f32* startPos, f32* endPos, int count, void* results, int flags) {
    ...
    for (i = 0; i < count; i++) {
        if (r->objects[i] != NULL) {
            Obj_TransformLocalVectorByWorldMatrix(r->objects[i], r->planes[i], r->planes[i]);
            if (contactSrc != NULL) {
                ObjHits_AddContactObject(r->objects[i], contactSrc);
            }
        }
    }
...
TrackTriangle* trackBuildModelTriangles(TrackTriangle* cur, TrackBlockDescriptor* desc, ModelFileHeader** model,
    ...
    for (; i < count; i++) {
        ...
        for (; t < tEnd; t++) {
            ...
            inv = 1.0f / len;
            cur->planeN[0] = fnx * inv;
            cur->planeN[1] = fny * inv;
            cur->planeN[2] = fnz * inv;

            if (flag8) {
                if (cur->planeN[1] >= 0.707f) {
                    continue;
                }
                if (cur->planeN[1] <= -0.707f) {
                    continue;
            ...
            if (flag4) {
                if (cur->planeN[1] < 0.707f && cur->planeN[1] > -0.707f) {
                    continue;
                }
            }
...

            {
                ...
                for (; j2 < 3; j2++) {
                    ...
                    px = cur->planeN[0] + xw[0];
                    py = cur->planeN[1] + yw[0];
                    pz = cur->planeN[2] + zw[0];
            ...
            *(s8*)&cur->surfaceType = (u8)trackGetPackedSurfaceType((int*)blk);
            cur->minMaxY = (u8)((maxYi << 4) | minYi);
            cur->flags = 10;
            cur->flags |= 8;
            cur++;
...
TrackTriangle* trackBuildBlockTriangles(TrackTriangle* cur, int x0, int y0, int z0, int x1, int y1, int z1, int flags,
    ...
    for (; i < count; i++) {
        ...
        tri = tri0;
        triEnd = tri0 + blk->polyGroupCount * 0x14;
        mask16 = mask;
        for (; tri < triEnd; tri += 0x14) {
            ...
            for (; t0 < vEnd; t0++, vq += 8) {
                ...
                vp = (s16*)(uintptr_t)(vb + ((MapTriIndex*)vq)->vert[0] * 6);
                minX = vp[0] >> 3;
                maxX = minX;
                ...
                cur->vy[0] = minY;
                cur->vz[0] = minZ + dzoff;
                vertices[0][0] = __OSs16tof32(&cur->vx[0]);
                vertices[0][1] = __OSs16tof32(&cur->vy[0]);
                vertices[0][2] = __OSs16tof32(&cur->vz[0]);
                maxYi = 0;
                minYi = 0;
                j = 1;
                ...
                for (; j < 3; j++) {
                    ...
                    cur->vy[j] = yy;
                    cur->vz[j] = z + dzoff;
                    vertices[j][0] = __OSs16tof32(&cur->vx[j]);
                    vertices[j][1] = __OSs16tof32(&cur->vy[j]);
                    vertices[j][2] = __OSs16tof32(&cur->vz[j]);
                    tw++;
                }
                if (minY > y1) {
                    continue;
                ...
                PSVECSubtract((Vec*)vertices[0], (Vec*)vertices[1], (Vec*)edges[0]);
                PSVECSubtract((Vec*)vertices[1], (Vec*)vertices[2], (Vec*)edges[1]);
                PSVECCrossProduct((Vec*)edges[0], (Vec*)edges[1], (Vec*)cur->planeN);
                mag = PSVECMag((Vec*)cur->planeN);
                if (!(mag > 0.0f)) {
                    continue;
                }
                mag = 1.0f / mag;
                PSVECScale((Vec*)cur->planeN, (Vec*)cur->planeN, mag);
                if (f8) {
                    if (cur->planeN[1] >= 0.707f || cur->planeN[1] <= -0.707f) {
                        if (type != 4) {
                            continue;
                ...
                cur->planeD = -PSVECDotProduct((Vec*)cur->planeN, (Vec*)vertices[0]);
                if (doEdges) {
                    int k22, deg, j2;
                    f32* ep;
                    f32 one, eps;
                    PSVECSubtract((Vec*)vertices[2], (Vec*)vertices[0], (Vec*)edges[2]);
                    k22 = 0;
                    deg = 0;
                    j2 = 0;
                    ep = edges[0];
                    ...
                    do {
                        f32 m;
                        PSVECCrossProduct((Vec*)cur->planeN, (Vec*)ep, (Vec*)en);
                        m = PSVECMag((Vec*)en);
                        if (m > eps) {
                            m = one / m;
...
void trackIntersectBroadphase(GameObject* obj, TrackQueryBounds* ranges, u32 queryMask, int b) {
...

    {
        ...
        gTrackTriangleBufferEnd = gTrackTriangleBuffer + 1200;
        masked = queryMask & 0xffff;
        if ((masked & 0x10) != 0) {
            cur = gTrackTriangleBuffer;
        } else {
            cur = trackBuildBlockTriangles(gTrackTriangleBuffer, x0, y0, z0, x1, y1, z1, queryMask, b);
        }
        if (cur < gTrackTriangleBufferEnd && (masked & 1) && obj != NULL) {
            ...
            for (; i < count; resetObjects++, i++) {
                ...
                desc->firstTriangle = (s16)(cur - gTrackTriangleBuffer);
        ...
        gTrackTriangleCount = (s16)(cur - gTrackTriangleBuffer);
...
void trackGetTriangleBuffer(int* outCount, TrackTriangle** outTable) {
...
void trackInitCollisionBuffers(void) {
    ...
    if (gTrackTriangleBuffer == NULL) {
        ...
        gIntersectLinePool = mmAlloc(0x5dc0, 0xffff00ff, 0);
        gIntersectPoints = mmAlloc(0x4fb0, 0xffff00ff, 0);
        gIntersectLineIndexTable = mmAlloc(0xbb8, 0xffff00ff, 0);
