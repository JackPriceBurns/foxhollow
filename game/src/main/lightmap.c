...
void sceneDraw(void) {
    ...
    if (gCloudLayerTexture != 0) {
        ...
        cloudMtx[10] = 0.0f;
        cloudMtx[11] = 1.0f;
        PSMTXConcat((MtxPtr)cloudMtx, (MtxPtr)Camera_GetInverseViewMatrix(), (MtxPtr)cloudMtx);
    }
    ...
    gLightmapDrawQueueCount = gLightmapDrawQueueCount + 1;
    sceneDrawTransparentPolys();
    (*gModgfxInterface)->markSourceFrameUpdated();
    ...
    if (player != NULL) {
        ...
        for (; i < player->childCount; i++) {
            ...
            if (child->anim.classId == 45) {
                STAFF_INTERFACE(child)->hitDetectGeometry(child);
            }
        }
    }
