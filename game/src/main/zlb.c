...
int zlbDecompress(u8 *compressedData, int compressedSize, u8 *destination, void *decompressedSize) {
    unsigned long outLen = *(u32*)decompressedSize;
    int r = uncompress(destination, &outLen, compressedData, (unsigned long)compressedSize);
    *(u32*)decompressedSize = (u32)outLen;
    return r == 0 ? (int)outLen : 0;
}
