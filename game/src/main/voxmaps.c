...
void voxmaps_visitRouteNeighbor(struct RouteState* state, VoxBoxArg* srcBox, int parentNodeIndex, u16 count, s16* box) {
    ...
    for (; i < 3; i++) {
        ...
        if (((((u8*)map + map->bitmap)[(ySlot << 5) | bitmapCol] >> shift) & 1) != 0u) {
            u8* node = voxmaps_getRouteNode((u8*)map + map->rowCounts, (int*)((u8*)map + map->nodeBase),
                                            (u8*)map + map->bitmap, voxX, ySlot, voxZ);
        ...
        if (map != NULL) {
            ...
            for (row = 0, p = &buf[0][0], bitmapCol = (voxZ << 1) + (voxX >> 3); row < 3; row++) {
                ...
                if (((((u8*)map + map->bitmap)[(slot << 5) | bitmapCol] >> voxXand7) & 1u) != 0u) {
                    node = voxmaps_getRouteNode((u8*)map + map->rowCounts, (int*)((u8*)map + map->nodeBase),
                                                (u8*)map + map->bitmap, voxX, slot, voxZ);
                    p[0] = (node[z6lo] >> shiftLo) & 3;
                    p[1] = (node[z6lo] >> shiftHi) & 3;
                    p[2] = (node[z6hi] >> shiftLo) & 3;
                    p[3] = (node[z6hi] >> shiftHi) & 3;
            ...
            if (map != NULL) {
                ...
                }
                {
                    ...
                    if (bit != 0) {
                        ...
                        if (routeNodeDirty != 0) {
                            routeNode =
                                voxmaps_getRouteNode((u8*)map + map->rowCounts, (int*)((u8*)map + map->nodeBase),
                                                     bitmap, tileX, ySlot, tileZ);
                            routeNodeDirty = 0;
                        }
