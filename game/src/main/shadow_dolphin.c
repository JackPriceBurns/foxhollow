...
static void buildGroundShadowQuad(s16* out, GameObject* obj) {
    ...
    if (trackGetNearestGroundOffsetAndNormal(obj, obj->anim.localPosX, obj->anim.localPosY, obj->anim.localPosZ, &dist,
                                             (f32*)&a, 0) == 0) {
        PSVECNormalize(&a, &a);
        b.x = 1.0f;
        b.y = 0.0f;
...
int objShadowRender(GameObject* obj, int renderMode, int unused, int frameCount) {
    ...
    if (shadowMesh == NULL || shadowMesh == OBJECT_SHADOW_MESH_UNCACHED) {
        ...
        idxOut = collectShadowTrackTriangles(obj, triangleBuffer, gShadowDrawScratch, (uintptr_t)gShadowVolumeBuffer,
