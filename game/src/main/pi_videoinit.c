...
char lbl_8030C6A0[0x1B8] = "Suspected graphics hang or infinite loop\0\0\0\0"
...
void waitNextFrame(void) {
    ...
    gFrameElapsedMs = (u64)OSCheckStopwatch(&gFrameStopwatch) / (f32)(u32)((OS_BUS_CLOCK >> 2) / 1000);
    ...
    {
        extern void fhDumpNearbyObjects(void);
        fhDumpNearbyObjects();
    }
    dt = timeDelta;
    if (dt > 0.1f) {
