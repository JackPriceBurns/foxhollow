...
u8 lbl_803DCC35;
u8 lbl_803DCC34;
uintptr_t gObjCachedModel;
uintptr_t gObjCachedTexture;
...
static void renderOpMatrix(ModelFileHeader* hdr, int* model, MtxBitStream* bs, f32* m1, f32* mtx, u8 nrm, u8 tex,
                           u8 skip) {
    u8* posMtxIds[1];
    char* cache;
    posMtxIds[0] = gObjGxPosMtxIdTable;
...
static void modelRenderFn_setVtxDescr(u8* modelHeader, u8* shader, ModelRenderOpTextureRefs* textureRefs,
                                      MtxBitStream* bitStream, u8 passMask, u8* usesNormalMatrix,
        ...
        }
        {
            ...
            for (; i < ((ModelFileHeader*)modelHeader)->texMtxCount; i++) {
                ...
                if (passMask == 4 && i == 0) {
                    int projectionModeB;
                    int projectionModeA;
                    if (gObjSelectedLightCount != 0 && (modelLightStruct_getProjectionTevModes(
...
static u8 addShaderLayerStages(GameObject* obj, Shader* shader, ModelRenderOpTextureRefs* p3, int mask, int p5,
                               int p6) {
    u16 alpha;
    u8* colp;
    void* tex;
        ...
        for (; layerIdx < shader->layerCount; layerIdx++) {
            ...
            if ((layer->typeBits & 0x80) == mask) {
                ...
                if (layer->texture != NULL) {
                    ...
                    if (layerIdx == 0) {
                        if ((p3->texture0 != NULL || p3->texture1 != NULL || p6 != 0) && ok) {
                            fl = 8;
                        } else {
                            fl = 0;
                        }
                    ...
                    color[2] = 0xff;
                    if (p3->texture0 != NULL ||
                        (shaderBytes[0] == 0xff && shaderBytes[1] == 0xff && shaderBytes[2] == 0xff)) {
                        ...
                        if (shader->vtxAttrFlags & 0x10) {
                            ...
                            addTexLayerStageSwizzled(tex, mtxp, (u8)fl, &gObjCurChanColor, p3->swapSelector, 1);
                        }
                    } else {
                        if (shader->vtxAttrFlags & 0x10) {
...
static u32 objSetupRenderOpGxState(GameObject* obj, u8* p2, int* am, MtxBitStream* bs) {
    ...
    if (refs->texture0 != NULL) {
        addSphereMapTexStage(refs->texture0, obj->sphereMapIntensity);
    }
    if (refs->texture1 != NULL) {
        if (op->unk1C != 0) {
            color[0] = 0xff;
        ...
        GXSetTevColor(GX_TEVREG2, *(GXColor*)color);
        {
            ...
            addLightTexReg2Stage(refs->texture1, hasBaseTexture, op->reg2TexSlot);
        }
        if (color[3] != 0) {
            u8 hasBaseTexture;
            if (refs->texture0 != NULL) {
...
static void modelDoAltRenderInstrs(GameObject* obj, GameObject* obj2, u8* m, int p4) {
    ...
    if (!(((ObjModel*)am)->bufferFlags & 8)) {
        ...
        if (((ModelFileHeader*)m)->animationCount != 0 && !(((ModelFileHeader*)m)->flags & 2) &&
            ...
            if (gObjCachedModel != (uintptr_t)m) {
    ...
    if (gObjRenderSetupDone == 0 || cb != NULL) {
        ...
        if (cb == NULL || cb((int*)obj, am, 0) == 0) {
            _gxSetFogParams();
            Rcp_ResetTextureStageState();
            addTexLayerStageSwizzled(textureIdxToPtr((uintptr_t)((ModelFileHeader*)m)->renderOps->layers[0].texture),
    ...
    if (!(((ObjModel*)am)->bufferFlags & 8)) {
        ...
        if (did != 0) {
            ...
            if (((ObjModel*)am)->vtxBufDirty != 0) {
                vtx = ((ObjModel*)am)->vtxBuf[(((ObjModel*)am)->bufferFlags >> 1) & 1];
            } else {
                vtx = (u8*)((ModelFileHeader*)m)->vertices;
            }
            ObjModel_BlendVertexStream((u8*)gObjBoneMtxBuffer, (ModelFileHeader*)m, vtx,
                                       ((ObjModel*)am)->vertexAnimData,
                                       ((ObjModel*)am)->vtxBuf[(((ObjModel*)am)->bufferFlags >> 1) & 1]);
            ObjModel_BlendNormalStream((u8*)gObjBoneMtxBuffer, (ModelFileHeader*)m, ((ModelFileHeader*)m)->normals,
...
void objRenderShadow(GameObject* obj) {
    ...
    if (obj->anim.classId == 1) {
        ...
        for (; i < obj->childCount; i++) {
            GameObject* child = obj->childObjs[i];
            if (child != NULL) {
