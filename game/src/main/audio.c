...
void AudioAramReadAllocAsync(u32 source, u32 size, void** outBuf, AudioArqRequestCallback callback,
    ...
    *outBuf = buf;
    ...
    ARQPostRequest(&entry->request, 0x64, 1, 1, source, (uintptr_t)buf, size, AudioAramReadCompleteCallback);
}
...
void AudioAramWriteSync(void* addr, u32 dest, u32 size) {
    ...
    gAudioArqRequestDone = 0;
    ARQPostRequest(&entry->request, 0x64, 0, 1, (uintptr_t)addr, dest, size, AudioAramWriteCompleteCallback);
...
int audioInit(void) {
    ...
    if (!gAudioInitStarted) {
        ...
        testAndSet_onlyUseHeap3(1);
        gAudioPendingLoadFlags |= AUDIO_LOAD_M_POOL;
        gAudioStarfoxMPoolDataHandle = loadFileByPathAsync("/audio/starfoxm.poo", NULL, 0, poolDataMLoadedCallback);
        gAudioPendingLoadFlags |= AUDIO_LOAD_M_PROJECT;
    ...
    if (!gAudioMusicGroupReady && (gAudioCompletedLoadFlags & AUDIO_LOAD_M_POOL) &&
        ...
        testAndSet_onlyUseHeap3(1);
        gAudioPendingLoadFlags |= AUDIO_LOAD_S_POOL;
        gAudioStarfoxSPoolDataHandle = loadFileByPathAsync("/audio/starfoxs.poo", NULL, 0, poolDataSLoadedCallback);
        gAudioPendingLoadFlags |= AUDIO_LOAD_S_PROJECT;
...
void Music_LoadChannelForTrigger(MusicTrigger* trigger) {
    ...
    AudioAramReadAllocAsync(slot->offset, slot->size, &channel->bankData, Music_ChannelLoadedCallback, slot, channel,
                            trigger);
}
