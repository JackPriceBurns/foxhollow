...
static void ObjHits_TraceSharpClawAttack(GameObject* attacker, GameObject* target, u32 mask, u32 previousMask,
    ...
    if ((attackerState->secondaryShapeFlags & OBJHITS_SHAPE_MODEL_HIT_VOLUMES) == 0 ||
        (((ObjHitsPriorityState*)target->anim.hitReactState)->secondaryShapeFlags & OBJHITS_SHAPE_MODEL_HIT_VOLUMES) ==
            0) {
        return;
    }
    ...
    previousSpheres =
        (ObjModelHitSphere*)attackerModel->hitVolumeSphereBuffers[((attackerModel->bufferFlags >> 2) & 1) ^ 1];
    ...
    for (i = 0; i < attackerCount; i++) {
        ...
        if (i != attackerVolumes[i].sphereIndex || (mask & (1U << (u8)attackerVolumes[i].maskBit)) == 0) {
            continue;
        }
...
int ObjHits_CalcSkeletonResponseXZ(f32* pos, f32 radius, GameObject* obj, ObjHitsSkeletonHit* hits,
                                   ObjHitsSkeletonJointData* jointPoints, ObjHitsModelFileHeader* jointModel,
...
void ObjHits_CheckSkeletonPair(GameObject* objA, GameObject* objB, void* hits, void* scratchB, void* scratchC,
    ...
    if ((shapeFlags & OBJHITBOX_SHAPE_SKELETON_3D) != 0) {
        ...
        point3D = point;
        hitCount = ObjHits_CollectSkeletonHits3D(&point3D.x, objBState->primaryRadius, hitboxBuf->jointWorkspace,
        ...
        point.z = objB->anim.worldPosZ - playerMapOffsetZ;
        pointXZ = point;
        hitCount = ObjHits_CollectSkeletonHitsXZ(&pointXZ.x, objBState->primaryRadius, hitboxBuf->jointWorkspace,
                                                 hitboxBuf, (ObjHitsSkeletonHit*)hits, &bestHit,
                ...
                if ((i == hitVolumes[i].sphereIndex) && ((mask2 & 1 << hitVolumes[i].maskBit) != 0)) {
                    ...
                    if (bits != 0) {
                        ...
                        if (pointCount < 4) {
                            ...
                            startPoints[pointCount * 3] = playerMapOffsetX + prevSpheres[i * 4 + 1];
                            startPoints[pointCount * 3 + 1] = prevSpheres[i * 4 + 2];
                            startPoints[pointCount * 3 + 2] = playerMapOffsetZ + prevSpheres[i * 4 + 3];
...
void ObjHits_Update(int objectCount) {
    ...
    for (; slotIndex < slotCount; entrySlot++, slotIndex++) {
        ...
        if ((attachedObj != 0) &&
            ((ObjAnim_GetPriorityHitState(&attachedObj->anim) == NULL) ||
             ((ObjAnim_GetPriorityHitState(&attachedObj->anim)->flags & OBJHITS_PRIORITY_STATE_ENABLED) == 0))) {
            attachedObj = 0;
        }
                    ...
                    if ((slotIndex != candidateIndex) && (obj->anim.parent != candObj)) {
                        ...
                        if (diff < objState->primaryRadiusXZ + candState->primaryRadiusXZ) {
                            ...
                            if ((diff < objState->primaryRadiusY + candState->primaryRadiusY) &&
                                ...
                                ((obj->anim.modelInstance->runtimeSourceHitMask & candState->targetMask) != 0) &&
                                ((candObj->anim.modelInstance->runtimeSourceHitMask & objState->targetMask) != 0)) {
                        ...
                        if (diff < objState->secondaryRadiusXZ + candState->secondaryRadiusXZ) {
                            axisDiff = (obj->anim.worldPosY - candObj->anim.worldPosY > gObjHitsScalarZero[0])
    ...
    for (slotIndex = 1, entrySlot = entrySlotBase; slotIndex < slotCount; entrySlot++, slotIndex++) {
        obj = (*entrySlot)->obj;
        if ((((ObjHitsPriorityState*)obj->anim.hitReactState)->flags & OBJHITS_PRIORITY_STATE_TRACK_CONTACT) != 0) {
    ...
    for (hitVolumeIndex = 0; hitVolumeIndex < 5; hitVolumeIndex++) {
        ...
        if (hitState != NULL) {
            ...
            if (stateActive != 0) {
                ...
                if (resetPending != 0) {
                    ...
                    priorityState->flags = (s16)(priorityState->flags & ~OBJHITS_PRIORITY_STATE_PAIR_RESPONSE_APPLIED);
    ...
    if (moveEntryTable != NULL) {
        for (moveEntryWordIndex = 0, moveEntry = moveEntryTable;
            ...
            if (moveId == ((ObjHitReactMoveEntry*)moveEntry)->moveId) {
                ...
                if (ObjHitReact_GetActiveEntryByteCount(hitState) > ObjHitReact_GetEntryBufferByteCapacity(hitState)) {
                    ObjHitReact_SetActiveEntryByteCount(hitState, ObjHitReact_GetEntryBufferByteCapacity(hitState));
                }
                if (async == 0) {
...
uintptr_t ObjHits_AllocObjectState(GameObject* obj, uintptr_t arena) {
    uintptr_t stateArena;
    ObjHitsPriorityState* hitState;

    stateArena = (arena + 3) & ~(uintptr_t)3;
...
int ObjHits_RecordObjectHit(GameObject* obj, GameObject* hitObj, s8 priority, int hitVolume, s8 sphereIndex) {
    ...
    if (hitObj != NULL && hitObj->anim.hitReactState != NULL) {
        ((ObjHitsPriorityState*)hitObj->anim.hitReactState)->lastHitObject = (uintptr_t)obj;
    }
    ...
    while (hitSlot < hitState->priorityHitCount) {
        if ((void*)hitState->hitObjects[hitSlot] == (void*)hitObj) {
            if (hitState->priorities[hitSlot] > priority) {
                ...
                hitState->hitPosX[hitSlot] = obj->anim.localPosX;
                hitState->hitPosY[hitSlot] = obj->anim.localPosY;
                hitState->hitPosZ[hitSlot] = obj->anim.localPosZ;
            }
    ...
    if (hitSlot == hitState->priorityHitCount && hitState->priorityHitCount < OBJHITS_PRIORITY_HIT_COUNT) {
        ...
        hitState->hitObjects[hitState->priorityHitCount] = (uintptr_t)hitObj;
        hitState->hitPosX[hitState->priorityHitCount] = obj->anim.localPosX;
        hitState->hitPosY[hitState->priorityHitCount] = obj->anim.localPosY;
        hitState->hitPosZ[hitState->priorityHitCount] = obj->anim.localPosZ;
...
int ObjHits_RecordPositionHit(GameObject* obj, GameObject* hitObj, s8 priority, int hitVolume, s8 sphereIndex,
    ...
    if ((targetObj != NULL) && (targetObj->hitReactState != NULL)) {
        ((ObjHitsPriorityState*)targetObj->hitReactState)->lastHitObject = (uintptr_t)obj;
    }
    hitSlot = 0;
...
void ObjHits_AddContactObject(GameObject* obj, GameObject* contactObj) {
    ...
    contactObjectCount = transformState->contactObjectCount;
    if (contactObjectCount >= OBJHITBOX_CONTACT_OBJECT_COUNT) {
        return;
    }
    for (i = 0; i < contactObjectCount; i++) {
        if (transformState->contactObjects[i] == contactObj) {
            return;
        }
    }
    contactObjectIndex = transformState->contactObjectCount++;
    transformState->contactObjects[contactObjectIndex] = contactObj;
}
