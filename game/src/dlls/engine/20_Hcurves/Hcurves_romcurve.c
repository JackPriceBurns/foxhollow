...
int RomCurve_getUnblockedControlPointId(RomCurveDef* curve, int exclude, int pickIdx);
...
int RomCurve_getControlPointId(RomCurveDef* curve, int exclude, int pickIdx)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (neighbor > -1 && ((s32)curve->blockedLinkMask & mask) != 0 && neighbor != exclude)
        {
...
static inline int RomCurve_hasNoOpenLink(RomCurveDef* curve)
{
    ...
    for (k = 0; k < 4; k++)
    {
        if (curve->linkIds[k] != -1 &&
            ((s8)curve->blockedLinkMask & (1 << k)) == 0)
        {
            return 0;
        }
    }
...
int RomCurve_func13(u32 curveId, int typeFilter, int matchValue, int* outLink)
{
    ...
    for (; li < 4; li++)
    {
        ...
        visited[startIdx] = 1;
        linkNode = RomCurve_findByIdWithIndex(start->linkIds[li], &idx);
        if (linkNode == NULL)
        {
            continue;
        }
...
int RomCurve_findLinkTowardNearestOfType(RomCurveDef* curve, int typeFilter, int actionFilter, int* previousCurveId)
{
    ...
    for (; li < 4; li++)
    {
        ...
        visited[startIdx] = 1;
        linkNode = RomCurve_findByIdWithIndex(curve->linkIds[li], &idx);
        if (linkNode == NULL)
        {
            continue;
        }
...
static inline int RomCurve_noUnblockedLinks(RomCurveDef* curve)
{
    ...
    {
        if ((s32)*lp++ != -1 && (curve->blockedLinkMask & (1 << bit)) == 0)
        {
            return 0;
        }
    }
    ...
    if (!done)
    {
        while (curve != NULL && !RomCurve_noUnblockedLinks(curve))
        {
            ...
            if (next != NULL)
            {
                ...
                tz = next->z;
                outZ[n++] = tz;
                outX[n] = ROMCURVE_TANGENT_SCALE * ((f32)curve->tangentMag * mathSinf(ROMCURVE_PLACEMENT_ANGLE(curve->yaw)));
