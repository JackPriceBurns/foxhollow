...
int Minimap_update(void) {
    ...
    if (player != NULL) {
        ...
        if ((*gCameraInterface)->getMode() == CAMERA_MODE_VIEWFINDER_RESOURCE_ID ||
            (gMinimapEnabled == 0 && gMinimapHelpTextActive == 0) || Camera_GetViewportYOffset() != 0 ||
            (player->objectFlags & OBJECT_OBJFLAG_PARENT_SLACK) != 0 || objIsCurModelNotZero(player) == 0 ||
            pauseMenuState != 0 || gTimeListPromptSelection != 0) {
            mapTextureId = 0;
        ...
        if (gMinimapFadeAlpha != 0) {
            ...
            if (gMinimapViewMode == MINIMAP_VIEW_MODE_AREA_NAME && gMinimapAreaNameAlpha != 0 &&
            ...
            case MINIMAP_VIEW_MODE_MAP:
                if (minimapTexture != NULL) {
                    ...
                    drawPartialTexture(minimapTexture, (50.0f - panx) - frac, ((f32)(int)gMinimapBoxY - pany) - fv,
                                       gMinimapContentAlpha & 0xff, (int)(256.0f * gMinimapZoom), texW - mapTileU,
                                       texH - mapTileV, mapTileU, mapTileV);
                    ...
                    {
                        f32 s1 = gMinimapArrowScale0 * mathSinf(3.1415927f * (f32)player->anim.rotX / 32768.0f);
                        f32 c1 = gMinimapArrowScale0 * mathCosf(3.1415927f * (f32)player->anim.rotX / 32768.0f);
                        f32 c2 =
                            gMinimapArrowScale1 * mathSinf(3.1415927f * (f32)(player->anim.rotX + 0x6000) / 32768.0f);
                        f32 s2 =
                            gMinimapArrowScale1 * mathCosf(3.1415927f * (f32)(player->anim.rotX + 0x6000) / 32768.0f);
                        f32 c3 =
                            gMinimapArrowScale2 * mathSinf(3.1415927f * (f32)(player->anim.rotX - 0x6000) / 32768.0f);
                        f32 s3 =
                            gMinimapArrowScale2 * mathCosf(3.1415927f * (f32)(player->anim.rotX - 0x6000) / 32768.0f);
                    ...
                    color.channels.b = 0;
                    {
                        ...
                        f32 c2 = -4.0f * mathSinf(3.1415927f * (f32)(player->anim.rotX + 0x6000) / 32768.0f);
                        f32 s2 = -4.0f * mathCosf(3.1415927f * (f32)(player->anim.rotX + 0x6000) / 32768.0f);
                        f32 c3 = -4.0f * mathSinf(3.1415927f * (f32)(player->anim.rotX - 0x6000) / 32768.0f);
                        f32 s3 = -4.0f * mathCosf(3.1415927f * (f32)(player->anim.rotX - 0x6000) / 32768.0f);
                        hudDrawTriangle(cx - s1, cy - c1, cx - c2, cy - s2, cx - c3, cy - s3, color.channels);
                    }
                } else {
            ...
            drawTexture(gMinimapCompassTexture, 32.0f, (f32)(int)(gMinimapBoxY - 0x14), gMinimapFadeAlpha & 0xff,
                        0x100);
            if (gMinimapFadeAlpha != 0) {
                ...
                xc = (s16)(gMinimapBoxY - 4);
                {
                    if (gMinimapViewMode == MINIMAP_VIEW_MODE_MAP && minimapTexture != NULL) {
                        if (gMinimapZoom < gMinimapMaxZoom) {
                            t = (f32)(sv = xc - 0x14);
                            hudDrawTriangle(44.0f, t, 52.0f, (f32)sv, 48.0f, (f32)(xc - 0x1a), compassColor.channels);
                        }
...
void Minimap_drawCompassNeedle(void) {
    ...
    y = gMinimapBoxY + 0x32;
    hudDrawTriangle(110.0f - c0, y - s0, 110.0f - c1, y - s1, 110.0f - cc2, y - s2, color.channels);
}
...
void Minimap_setupCompassBlip(void) {
    ...
    scale = 0.05f;
    for (; i < 2; i++) {
        gMinimapBlipObjects[i] =
            (GameObject*)objSetupObject(Obj_AllocObjectSetup(32, MINIMAP_COMMAND_MENU_OBJ_BASE + i), 4, -1, -1, 0);
    ...
    if (player == NULL || (*gCameraInterface)->getMode() == CAMERA_MODE_VIEWFINDER_RESOURCE_ID ||
        ...
        if (gMinimapEnabled == 0 && gMinimapHelpTextActive == 0) {
            ...
            case MINIMAP_VIEW_MODE_RADAR:
                ...
                gMinimapRadarTarget = objGetNearestTypeTo(FUEL_CELL_OBJECT_GROUP, player, &dist);
                if ((void*)gMinimapRadarTarget != NULL) {
                    ...
                    targetAngle = getAngle(gMinimapRadarTarget->anim.localPosX - player->anim.localPosX,
