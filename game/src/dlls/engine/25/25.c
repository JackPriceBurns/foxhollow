...
void dll_19_changeWeapon(GameObject* cam, u8* ctx) {
    ...
    if (Obj_IsLoadingLocked() != 0) {
        if ((s8)ctx[1031] > 0) {
            ...
            cam->childObjs[0] = objSetupObject(setup, 4, -1, -1, cam->anim.parent);
...
void dll_19_initGroundBaddie(GameObject* obj, GroundBaddiePlacement* config, u8* state, int moveArg0, int moveArg1,
    ...
    if (state->glowAlpha > 0.0f) {
        ...
        if ((state->flags400 & DLL19_FLAG_OSC_RISING) != 0) {
            ...
            if (state->glowAlpha > 2.0f) {
                ...
                obj->anim.flags = obj->anim.flags | OBJANIM_FLAG_HIDDEN;
                (*gMapEventInterface)
                    ->addTime(other->base.ident,
                              (f32)(s32)(ObjAnim_ReadPlacementS16(&obj->anim, &(other->respawnDelay)) * 60));
            }
        } else {
            if (state->glowAlpha < 0.0f) {
                state->glowAlpha = 0.0f;
...
void dll_19_startHitReaction(GameObject* obj, void* state, void* hitbox, s16 gameBit, u8* flagOut, s16 substate,
            ...
            if ((s8)((BaddieState*)state)->hitPoints != 0) {
                ...
                if (Player_GetCurrentHealth((uintptr_t)obj) <= 0) {
                    ...
                    if (losOut == 1 || traced != 0) {
                        if (trackGetLineIntersect(&self->anim.localPosX, gridIn, 1.0f, 0, &bboxOut, self, 4, -1, 0,
                                                  0) != 0) {
                            found = 0;
                        }
                    } else {
    ...
    if (((BaddieState*)state)->moveDone != 0) {
        if (((BaddieState*)state)->targetObj == player && (s8)((BaddieState*)state)->hitPoints != 0) {
            if (((BaddieState*)state)->targetDistance > distThreshold && requireFar != 0) {
                ...
                if (trackGetLineIntersect(&obj->anim.localPosX, pos, 1.0f, 0, (TrackBBoxHit*)out, obj, 4, -1, 0, 0) !=
                    0) {
                    result = 1;
                }
            }
...
u8 dll_19_getClearDirectionMask(GameObject* obj, void* state, f32 dist) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (ok != 0) {
            if (trackGetLineIntersect(&obj->anim.localPosX, world, 1.0f, 0, &bboxOut, obj,
                                      ((BaddieState*)state)->bboxTraceFlags, -1, 0, 0) != 0) {
                ok = 0;
            }
        }
        mask |= ok << i;
