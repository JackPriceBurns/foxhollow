...
int camcontrol_traceMove(f32* fromPos, f32* toPos, f32* outPos, CamcontrolTraceWork* traceWork, char traceMode,
                         u8 runTrace, u8 runBbox, f32 radius) {
    CamcontrolTraceWork* work;
    u8 blocked;
    int clear;
    f32 endTmp[3];
    ...
    *outPos = *toPos;
    ...
    outPos[2] = toPos[2];
    work->radii[0] = radius;
    work->surfaceTypes[0] = (u8)-1;
    work->queryTypes[0] = traceMode;
    work->hitCount = 0;
    blocked = 0;
    if (runBbox != 0) {
    ...
    if (runTrace != 0) {
        hitDetect_calcSweptSphereBounds(&sweptBounds, fromPos, outPos, work->radii, 1);
        trackIntersectBroadphase(NULL, &sweptBounds, 0x240, 1);
    }
    trackGetIntersect(NULL, fromPos, outPos, 1, traceWork, 0);
    clear = 0;
    if ((gCamcontrolTraceBboxBlocked == 0) && (work->hitCount == 0)) {
        clear = 1;
    }
    return clear;
}
...
u8 camcontrol_getTargetPosition(CameraObject* camera, ObjAnimComponent* targetAnim, f32* outPos, s16* outRotY) {
    ...
    camcontrol_traceMove(prev, pos, outPos, &traceWork, 3, '\x01', '\x01', 4.0f);
...
int CameraModeNormal_chooseWallAvoidanceDirection(CameraObject* cam, f32* outA, f32* outB, int angle) {
    ...
    memcpy(&probe[35], &tgt0, sizeof(tgt0));
    ...
    while ((s16)s <= 0x5a) {
        if (found1 == -1) {
            ...
            pA[4] = probe[1];
            pA[5] = probe[2];
            if (camcontrol_traceMove(prev, pp, NULL, &traceWork, 7, '\0', '\0', 3.9f) != 0) {
                found1 = i;
            }
        }
        if (found2 == -1) {
    ...
    if (found1 == -1) {
        ...
        for (i = 0; i <= found1; i++) {
            if (camcontrol_traceMove(pA0, pathA + (i + 1) * 3, NULL, &traceWork, 7, '\0', '\0', 3.9f) == 0) {
                found1 = 6;
                break;
            }
            pA0 = pA0 + 3;
    ...
    if (found2 == -1) {
        ...
        for (i = 0; i <= found2; i++) {
            if (camcontrol_traceMove(pB0, pathB + (i + 1) * 3, NULL, &traceWork, 7, '\0', '\0', 3.9f) == 0) {
                found2 = 6;
                break;
            }
            pB0 = pB0 + 3;
...
void CameraModeNormal_updateWallAvoidance(CameraObject* camera, GameObject* target) {
    ...
    trace = camcontrol_traceMove(prev, &camera->anim.worldPosX, NULL, &traceWork, 7, '\0', '\0', 3.9f);
    blocked = 0;
    if (trace == 0) {
        blocked = 1;
    }
...
void CameraModeNormal_updateVerticalBounds(CameraObject* camera, int flags, int collisionFlag, float* upperBound,
    ...
    camObj = (GameObject*)camera->anim.targetObj;
    if ((flags & 1) != 0) {
        float range = 4.0f;
        ...
        res = trackGetLineIntersect(&camera->probePos.x, &camera->anim.worldPosX, range, 1, NULL, NULL, 0x10,
                                    0xffffffff, 0xff, 0);
        camera->cameraCollisionActive = res;
        pos[0] = camera->anim.worldPosX;
        pos[1] = camera->anim.worldPosY;
        pos[2] = camera->anim.worldPosZ;
        hitDetect_calcSweptSphereBounds(&bounds, &camera->probePos.x, pos, &camera->collisionSweepRadius, 1);
        trackIntersectBroadphase(camObj, &bounds, 0x240, 1);
        trackGetIntersect(camObj, &camera->probePos.x, pos, 1, &traceHits, 0);
...
void CameraModeNormal_follow(CameraObject* camera, ObjAnimComponent* target) {
    ...
    ratio = PSVECMag((Vec*)&target->velocityX);
...
void CameraModeNormal_update(CameraObject* camera) {
    ...
    float dy;
    float dx;
    CamcontrolTraceWork wallTraceWork;
    CamcontrolTraceWork probeTraceWork;
    ...
    if (gCameraModeNormalState->wallAvoidanceFlags.active == 0) {
        ...
        if (((camera->cameraCollisionActive != 0) ||
             ((gCameraModeNormalState->targetActionFlags == 1 && (gCameraModeNormalState->traceNormalY >= 0.0f)))) &&
            (gCameraModeNormalState->clampFlags.distanceClamped == 0)) {
        ...
        if ((gCameraModeNormalState->traceHitMask == 1) && (gCameraModeNormalState->traceNormalY < fa)) {
            gCameraModeNormalState->wallAvoidanceFlags.active = 0;
        }
...
void CameraModeNormal_init(CameraObject* cam, int mode, CameraModeNormalInitSettings* settings) {
    ...
    case 4:
        ...
        cam->anim.rotZ = 0;
        cam->probePos.x = cam->anim.worldPosX;
        cam->probePos.y = cam->anim.worldPosY;
        cam->probePos.z = cam->anim.worldPosZ;
...
RESOURCE_ACQUIRE_ADAPTER(gCameraModeNormalDescriptorAcquire, CameraModeNormal_initialise)

CameraModeNormalDescriptor gCameraModeNormalDescriptor = {
    {{0x00000000, 0x00000000, 0x00000000, 0x000b0000}, gCameraModeNormalDescriptorAcquire, CameraModeNormal_release},
    NULL,
