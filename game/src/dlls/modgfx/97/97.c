...
STATIC_ASSERT(offsetof(Dll61EffectResourceView, colors) == 0x5C);
...
void dll_61_spawnEffect(GameObject* sourceObj, int variant, PartFxSpawnParams* spawnParams, u32 spawnFlags) {
    ...
    f32 randomScale;
    u8* resourceData = (u8*)gDll61EffectResourceData;
    GfxCmd* commands;
    ...
    commands[5].mode = 2;
    randomScale = 0.05f * (f32)randomGetRange(0, 0xc);
    ...
    packet.context.commandCount = (GfxCmd*)((u8*)commands + sizeof(GfxCmd) * 15) - commands;
    packet.context.sequenceParams[0] = *(s16*)&resourceData[offsetof(Dll61EffectResourceView, sequenceParams[0])];
    packet.context.sequenceParams[1] = *(s16*)&resourceData[offsetof(Dll61EffectResourceView, sequenceParams[1])];
    packet.context.sequenceParams[2] = *(s16*)&resourceData[offsetof(Dll61EffectResourceView, sequenceParams[2])];
    packet.context.sequenceParams[3] = *(s16*)&resourceData[offsetof(Dll61EffectResourceView, sequenceParams[3])];
    packet.context.sequenceParams[4] = *(s16*)&resourceData[offsetof(Dll61EffectResourceView, sequenceParams[4])];
    packet.context.sequenceParams[5] = *(s16*)&resourceData[offsetof(Dll61EffectResourceView, sequenceParams[5])];
    packet.context.sequenceParams[6] = *(s16*)&resourceData[offsetof(Dll61EffectResourceView, sequenceParams[6])];
    ...
    (*gModgfxInterface)
        ->spawnEffect(&packet.context, 0, 9, (u8*)gDll61EffectResourceData, 8,
                      &resourceData[offsetof(Dll61EffectResourceView, colors)], 0x90, 0);
}
...
RESOURCE_ACQUIRE_ADAPTER(gDll61ResourceDescriptorAcquire, dll_61_initialise)

Dll61ResourceDescriptor gDll61ResourceDescriptor = {
    { {0x00000000, 0x00000000, 0x00000000, 0x00030000}, gDll61ResourceDescriptorAcquire, dll_61_release },
    NULL,
    dll_61_spawnEffect,
};
