...
void DR_CageWith_free(GameObject* obj, int arg) {
    DrcagewithState* state = obj->extra;
    if (state->spawnedObject != 0 && arg == 0 && state->spawnedObject->anim.modelInstance != 0) {
    ...
    if (visible != 0) {
        ...
        if (state->spawnedObject != 0) {
            ObjPath_GetPointWorldPosition(obj, 0, &state->spawnedObject->anim.localPosX,
                                          &state->spawnedObject->anim.localPosY, &state->spawnedObject->anim.localPosZ,
                                          0);
            objRenderModelAndHitVolumes(state->spawnedObject, p2, p3, p4, p5, 1.0f);
            GameObject* linkedObj = state->linkedObject;
            if (linkedObj != 0) {
                ...
                objRenderModelAndHitVolumes(linkedObj, p2, p3, p4, p5, 1.0f);
            }
        }
    }
}
...
void DR_CageWith_hitDetect(GameObject* obj) {
    ...
    f32 maxDist = 300.0f;
    DrcagewithState* state = obj->extra;
    BitFlags8* bf31 = &state->ropeFlags;
    ...
    if (state->spawnedObject == NULL) {
        if (Obj_IsLoadingLocked()) {
            ObjPlacement* spawned = Obj_AllocObjectSetup(32, DRCAGEWITH_CHILD_OBJ);
            spawned->color[0] = 2;
            spawned->color[1] = 1;
            ...
            GameObject* child = objSetupObject(spawned, 5, (obj)->anim.mapEventSlot, -1, (obj)->anim.parent);
    ...
    if (bf31->b0 == 0) {
        if (mainGetBit(GAMEBIT_DR_RescuedCloudRunner) != 0) {
            ...
            (obj)->anim.flags |= OBJANIM_FLAG_HIDDEN;
            bf31->b0 = 1;
            GameObject* nearest = objGetNearestTypeTo(DRCAGEWITH_TARGET_OBJGROUP, obj, &maxDist);
        ...
        f32 angVel = oneOverTimeDelta * (obj->anim.localPosX - obj->anim.previousLocalPosX);
        angVel *= -1300.0f;
        angVel = interpolate(angVel - state->angularVel, 0.05f, timeDelta);
        state->angularVel += CLAMP_EXPR(angVel, -50.0f * timeDelta, 50.0f * timeDelta);
