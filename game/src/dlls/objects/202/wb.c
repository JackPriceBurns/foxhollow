...
void wbUpdateEngaged(GameObject* obj, void* state) {
    ...
    if ((((EnemyState*)state)->controlFlags & 0x8000000) != 0) {
        ...
        moveSpeed = sidekickToy_accelerateTowardTargetXZ((GameObject*)(obj), tracked->anim.worldPosX,
                                                         50.0f + tracked->anim.worldPosY, tracked->anim.worldPosZ,
                                                         30.0f, 0.2f, 5.0f, ((EnemyState*)state)->drag);
    }
...
void wbUpdateIdle(GameObject* obj, void* state) {
    ...
    if ((((EnemyState*)state)->controlFlags & BADDIE_CONTROL_PATH_FOLLOW) != 0) {
        if (((Curve_AdvanceAlongPath(&route->curve, ((EnemyState*)state)->pathStep) != 0 || route->curve.idx != 0) &&
             (*gRomCurveInterface)->goNextPoint(route) != 0) &&
            (*gRomCurveInterface)
                    ->initCurve(*(RomCurveWalker**)state, (void*)obj, 700.0f, (int*)&gWbCurveInitData, -1) != 0) {
            ((EnemyState*)state)->controlFlags = ((EnemyState*)state)->controlFlags & ~(u64)BADDIE_CONTROL_PATH_FOLLOW;
        }
        if ((((EnemyState*)state)->controlFlags & 0x8000000) != 0) {
            moveSpeed = 30.0f;
        } else {
            moveSpeed = sidekickToy_accelerateTowardTargetXZ((GameObject*)(obj), route->curve.sample[0],
                                                             route->curve.sample[1], route->curve.sample[2], 30.0f,
                                                             0.2f, 5.0f, ((EnemyState*)state)->drag);
        }
    } else if ((((EnemyState*)state)->controlFlags & 0x8000000) != 0) {
        moveSpeed = 30.0f;
    } else {
        moveSpeed =
            sidekickToy_accelerateTowardTargetXZ((GameObject*)(obj), placement->posX, placement->posY, placement->posZ,
                                                 30.0f, 0.2f, 5.0f, ((EnemyState*)state)->drag);
    }
    if (((moveSpeed > 0.0f) && (((GameObject*)obj)->anim.velocityY < -0.5f)) ||
        ((((EnemyState*)state)->controlFlags & 0x8000000) != 0)) {
