...
void hagabonMK2_updateB(GameObject* obj, u8* state)
{
    ...
    if ((((EnemyState*)state)->controlFlags & BADDIE_CONTROL_PATH_FOLLOW) != 0)
    {
        f32* dp = dv;
        f32 t;
        dp[0] = base->curve.sample[0] - obj->anim.worldPosX;
        dp[1] = base->curve.sample[1] - obj->anim.worldPosY;
        dp[2] = base->curve.sample[2] - obj->anim.worldPosZ;
        ...
        sidekickToy_accelerateTowardTarget3D(obj, base->curve.sample[0], base->curve.sample[1], base->curve.sample[2], 60.0f,
