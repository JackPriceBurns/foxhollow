...
void arwsquadron_followLeader(GameObject* obj, ArwSquadronState* state) {
    ...
    *(s16*)&state->swayPhaseY = state->swaySpeedY * timeDelta + state->swayPhaseY;
    ...
    out[0] = 15.0f * mathSinf(3.14159265f * state->swayPhaseX / 32768.0f) + 5.0f * setup->leaderOffsetX;
    out[1] = 15.0f * mathSinf(3.14159265f * state->swayPhaseY / 32768.0f) + 5.0f * setup->leaderOffsetY;
    ...
    if (!state->flags.cmd.rollCmdActive) {
        objAnim->rotZ = state->rollAmplitude * mathSinf(3.14159265f * state->swayPhaseX / 32768.0f) + leaderAnim->rotZ;
    }
...
void arwsquadron_followPath(GameObject* obj, ArwSquadronState* state) {
    ...
    pathResult = Obj_UpdateRomCurveFollowVelocity(obj, &state->curve, state->pathSpeed, 100.0f, state->pathSpeed, 1);
    if (pathResult == -1) {
        objAnim->flags |= OBJANIM_FLAG_HIDDEN;
        ObjHits_DisableObject(obj);
        state->phase = ARW_SQUADRON_STATE_DISABLED;
        ...
        if (setup->pathMode == 2) {
            if (state->variant == ARW_SQUADRON_VARIANT_SQUADRON) {
                Obj_SmoothTurnAnglesTowardVelocity(obj, (const Vec3f*)&objAnim->velocityX, 0xf, 50.0f, 1.0f);
            } else {
...
void arwsquadron_updateVolley(GameObject* obj, ArwSquadronState* state, ArwSquadronSetup* setup) {
    ...
    if (!flags->volleyInProgress) {
        ...
        s16toFloat(&state->shotIntervalTimer, setup->shotInterval);
        state->volleyAngle +=
            ObjAnim_ReadPlacementU16(&obj->anim, &(setup->volleyAngleSpread)) * 2 / setup->shotsPerVolley;
    ...
    case ARW_SQUADRON_STATE_WAITING: {
        ...
        if (ObjAnim_ReadPlacementS32(&obj->anim, &(placement->leaderObjectId)) > 0) {
            if (state->leaderObj == NULL) {
                state->leaderObj =
                    ObjList_FindObjectById(ObjAnim_ReadPlacementS32(&obj->anim, &(placement->leaderObjectId)));
            }
            leader = state->leaderObj;
        }
        if (leader != NULL && arwsquadron_isPlayerWithinRangeZ(leader, state->activationDistance) &&
            ((ObjAnim_ReadPlacementS16(&obj->anim, &(placement->gameBit)) <= 0 &&
              arwsquadron_isPlayerWithinRangeZ(leader, state->exitDistance)) ||
...
void ARWSquadron_init(GameObject* obj, ArwSquadronSetup* setup) {
    ...
    if (setupData->base.objectId == ARW_SQUADRON_BIGASTEROID_OBJ ||
        setupData->base.objectId == ARW_SQUADRON_SMALLASTEROID_OBJ) {
