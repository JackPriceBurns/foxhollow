        ...
        if (state->fadeTarget == 0.0f) {
            ...
            state->fadeTarget = 2.0f;
            {
                ...
                for (; i < SHIELD_SEGMENT_COUNT; i++) {
                    ...
                    state->segmentRate[i] = (s16)((f32)(int)(i * randomGetRange(SHIELD_SEGMENT_RATE_RANDOM_MIN,
...
void Shield_update(GameObject* obj) {
    ...
    }
    {
        ...
        i = 0;
        for (; i < SHIELD_SEGMENT_COUNT; i++) {
            state->segmentPhase[i] = (s16)((f32)state->segmentRate[i] * timeDelta + state->segmentPhase[i]);
            if (obj->anim.romDefNo == SHIELD_SEQID_OMNI_SHIELD) {
