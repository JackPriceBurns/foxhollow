...
static void LargeCrate_spawnPickup(GameObject* obj) {
    ...
    if (Obj_IsLoadingLocked() != 0) {
        childPlacement = (LargeCratePickupPlacement*)Obj_AllocObjectSetup(LARGECRATE_PICKUP_PLACEMENT_SIZE,
                                                                          LARGECRATE_CHILD_OBJECT_PICKUP);
        ...
        objSetupObject((ObjPlacement*)childPlacement, 5, obj->anim.mapEventSlot, -1, obj->anim.parent);
    }
}
...
int LargeCrate_spawnDropContents(GameObject* obj, GameObject* player, LargeCrateState* state) {
    ...
    case LARGECRATE_DROPTYPE_GREEN_SCARAB:
        ...
        horizontalMagnitude =
            child->anim.velocityX * child->anim.velocityX + child->anim.velocityZ * child->anim.velocityZ;
        ...
        child->anim.velocityX =
            child->anim.velocityX * -(0.01f * (f32)randomGetRange(0, LARGECRATE_CHILD_RANDOM_VELOCITY_MAX) - 1.0f);
        child->anim.velocityZ =
            child->anim.velocityZ * (1.0f - 0.01f * (f32)randomGetRange(0, LARGECRATE_CHILD_RANDOM_VELOCITY_MAX));
        ...
        angleDelta = child->anim.rotX - ((int)(s16)getAngle(child->anim.velocityX, -child->anim.velocityZ) & 0xFFFF);
    ...
    case LARGECRATE_DROPTYPE_APPLE:
        ...
        child = objSetupObject((ObjPlacement*)childPlacement, 5, obj->anim.mapEventSlot, -1, obj->anim.parent);
        (*(CollectibleInterface**)child->anim.dll)->startBounceMotion(child, 0.0f, 1.0f, 0.0f);
        break;
...
void LargeCrate_update(GameObject* obj) {
    ...
    if ((*gMapEventInterface)->shouldNotSaveTime(placement->base.ident) == 0) {
        ...
        hitKind = ObjHits_GetPriorityHitWithPosition(obj, &hitObject, &hitType, (u32*)&hitDamage, &effectParams.posX,
