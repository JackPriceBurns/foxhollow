...
typedef struct CfGuardianSeqStreamEntry {
    int questState;
    s32* sequences;
} CfGuardianSeqStreamEntry;

CfGuardianSeqStreamEntry gCfGuardianSeqStreamTable[CFGUARDIAN_SEQUENCE_TABLE_ENTRY_COUNT] = {
    {0, gCfGuardianState0Sequences},
    {1, gCfGuardianState1Sequences},
    {2, gCfGuardianState2Sequences},
    {3, gCfGuardianState3Sequences},
    {4, gCfGuardianState4Sequences},
    {5, gCfGuardianState5Sequences},
    {6, gCfGuardianState6Sequences},
    {7, gCfGuardianState7Sequences},
    {8, gCfGuardianState8Sequences},
    {9, gCfGuardianState9Sequences},
    {10, gCfGuardianState10Sequences},
    {12, gCfGuardianState12Sequences},
    {13, gCfGuardianState13Sequences},
    {14, gCfGuardianState14Sequences},
    {15, gCfGuardianState15Sequences},
};

static s32* cfguardianLookupSequences(int questState) {
    int i;

    for (i = 0; i < CFGUARDIAN_SEQUENCE_TABLE_ENTRY_COUNT; i++) {
        if (gCfGuardianSeqStreamTable[i].questState == questState) {
            return gCfGuardianSeqStreamTable[i].sequences;
        }
    }
    return NULL;
}
...
int cfguardian_updateMain(GameObject* obj) {
    ...
    if (ObjTrigger_IsSet(obj) != 0) {
        ...
        if ((*gGameUIInterface)->isItemBeingUsed(CFGUARDIAN_WATER_SPELL_STONE_EVENT) != 0) {
            ...
            int* sequenceChoices = cfguardianLookupSequences(state->questState);
            int pick;
