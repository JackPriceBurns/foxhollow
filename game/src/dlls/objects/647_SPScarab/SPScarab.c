...
void SPScarab_update(GameObject* obj) {
    ...
    objMove(obj, timeDelta * (obj->anim.velocityX * state->speedScale), obj->anim.velocityY * timeDelta,
            timeDelta * (obj->anim.velocityZ * state->speedScale));

    distance = sqrtf(obj->anim.velocityX * obj->anim.velocityX + obj->anim.velocityZ * obj->anim.velocityZ);
    ...
    if (trackGetLineIntersect(&obj->anim.previousLocalPosX, &obj->anim.localPosX, 3.0f, 0, &hit_buf, obj, 8, -1, 0xff,
                              0xa) != 0) {
    ...
    if (getXZDistanceSquared(&Obj_GetPlayerObject()->anim.worldPosX, &obj->anim.worldPosX) < 100.0f) {
    ...
    if ((obj->objectFlags & OBJECT_OBJFLAG_RENDERED) != 0) {
        if (state->burstCount != 0) {
            objfx_spawnDirectionalBurst(obj, 5, 1.0f, state->burstCount, 1, 0x14, 2.5f, NULL, 0);
        }
    }
}
