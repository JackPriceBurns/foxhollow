    ...
    if (phase == WCBEACON_PHASE_WAITING_FOR_TRICKY) {
        ...
        if (mainGetBit(ObjAnim_ReadPlacementS16(&obj->anim, &(setup->armBit))) == 0) {
            ...
            if (stayPoint != obj || Tricky_requestRecallAndCheckBusy(tricky) != 0) {
                (*gObjectTriggerInterface)->runSequence(WCBEACON_TRIGGER_RELEASE_SLOT, obj, WCBEACON_TRIGGER_NO_ARG);
                state->phase = WCBEACON_PHASE_IDLE;
            }
        } else {
            obj->anim.resetHitboxFlags &= ~INTERACT_FLAG_DISABLED;
            if (tricky != NULL && (obj->anim.resetHitboxFlags & INTERACT_FLAG_IN_RANGE)) {
                TRICKY_INTERFACE(tricky)->sideCommandEnable(tricky, obj, WCBEACON_COMMAND_KIND, WCBEACON_COMMAND_TYPE);
            }
        }
        ...
        if (obj->objectFlags & OBJECT_OBJFLAG_RENDERED) {
            (*gPartfxInterface)
                ->spawnObject(obj, WCBEACON_PARTFX_ACTIVE, NULL, WCBEACON_PARTFX_KIND, WCBEACON_TRIGGER_NO_ARG, NULL);
        }
        if (obj->userData1 == 0) {
