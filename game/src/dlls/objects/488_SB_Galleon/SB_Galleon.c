...
void SB_Galleon_updateFlight(GameObject* obj) {
    ...
    case 0:
        ...
        tz = 150.0f * mathCosf((3.14159265f * (f32)state->bobPhase) / 32768.0f) + state->homeZ;
        ty = 60.0f * mathSinf((3.14159265f * (f32)state->bobPhase) / 32768.0f) + (state->homeY - 300.0f);
    ...
    case 1:
        ...
        case 0:
            ...
            if ((state->headingLatch <= 0) && ((state->phaseCounter == 0) || (state->phaseCounter == 5))) {
                state->headingLatch = 200;
            }
    ...
    case 8: {
        ...
        dy = ty - state->posY;
        dz = tz - state->posZ;
        state->speed = state->speed + (speedTarget - state->speed) / 30.0f;
        ...
        state->turnRate = state->turnRate + ((framesThisStep * (diff - state->turnRate)) >> 4);
        c = state->phase;
        if ((c == 3) || (c == 4)) {
        ...
        Matrix_TransformPoint(mtx, 0.0f, 0.0f, -state->speed * timeDelta, &state->driftX, &state->driftY,
                              &state->driftZ);
        if (state->phase == 7) {
            state->posX = tx;
            state->posY = ty;
        ...
        obj->anim.localPosZ = state->posZ + state->swayZ + (tricky->anim.localPosZ - state->refZ);
        if (state->stage >= 7) {
    ...
    if (state->phase < 2) {
        state->posX = state->moveScale * (state->driftX * timeDelta) + state->posX;
        state->posY = state->moveScale * (state->driftY * timeDelta) + state->posY;
        state->posZ = state->moveScale * (state->driftZ * timeDelta) + state->posZ;
        state->moveScale += 0.004166667f;
        if (state->moveScale > 1.0f) {
            state->moveScale = 1.0f;
        }
        blendK = 0.02f;
        state->swayScaleSmooth += blendK * (timeDelta * (ambA - state->swayScaleSmooth));
        state->rollScaleSmooth += blendK * (timeDelta * (ambC - state->rollScaleSmooth));
        state->swayResponseSmooth += blendK * (timeDelta * (ambB - state->swayResponseSmooth));
        if (state->phase == 0) {
            zRatio = (f32)tricky->anim.rotY / state->swayScaleSmooth;
            state->swayZ += timeDelta * (state->swayResponseSmooth *
                                         ((f32)-tricky->anim.rotZ / state->swayScaleSmooth - state->swayZ));
            state->swayY += timeDelta * (state->swayResponseSmooth * (zRatio - state->swayY));
            zero = 0.0f;
            state->swayX = zero;
            state->swayY = zero;
            ...
            state->swayZ -= timeDelta * (state->swayZ * state->swayResponseSmooth);
            state->swayY -= timeDelta * (state->swayY * state->swayResponseSmooth);
            t = 0;
            wrap = t;
        }
        ...
        state->rollLatch = state->rollLatch + ((framesThisStep * (t - state->rollLatch)) >> 5);
...
void SB_Galleon_updateShield(GameObject* obj) {
    ...
    if (mainGetBit(DBPROTECTION_GAMEBIT_TRANSITION_ARMED) != 0 &&
        mainGetBit(DBPROTECTION_GAMEBIT_TRANSITION_USED) == 0 && mainGetBit(GAMEBIT_ITEM_WMGoldKey_Got) != 0) {
