...
void gunpowderBarrel_hitDetect(GameObject* barrel) {
    ...
    if (state->heldByCarryInterface == 0 &&
        trackGetLineIntersect(&barrel->anim.previousLocalPosX, &barrel->anim.localPosX, 8.0f, 1, &collision, barrel, 8,
                              -1, 0xff, 0) != 0) {
        if (collision.kind == 0x14) {
            state->detonationTrigger = GUNPOWDER_BARREL_DETONATION_TRIGGER_IMPACT;
        }

        if (state->heldFlags.playerHeld != 0 && collision.kind == 3) {
            gunpowderBarrel_setPlayerHeldState(barrel, 0);
            objFreeObjectType(barrel, GUNPOWDER_BARREL_LOOSE_OBJECT_GROUP);
        } else {
            collisionNormal[0] = collision.normalX;
            collisionNormal[1] = collision.normalY;
            collisionNormal[2] = collision.normalZ;
...
void gunpowderBarrel_init(GameObject* obj, GunpowderBarrelPlacement* placement) {
    ...
    state->carryable.flags |= CARRYABLE_FLAG_GRAVITY_DISABLED;
...
OBJECT_INIT_ADAPTER(gGunpowderBarrelObjDescriptorInitAdapter, gunpowderBarrel_init, obj, placement)
OBJECT_EXTRA_SIZE_ADAPTER(gGunpowderBarrelObjDescriptorExtraSizeAdapter, gunpowderBarrel_getExtraSize)

typedef struct GunpowderBarrelObjDescriptorTypeInterface {
    OBJECT_INTERFACE_FIELDS;
    __typeof__(gunpowderBarrel_addThrowVelocity)* gunpowderBarrel_addThrowVelocity;
} GunpowderBarrelObjDescriptorTypeInterface;

typedef struct GunpowderBarrelObjDescriptorTypeCore {
    ObjectDescriptorHeader header;
    GunpowderBarrelObjDescriptorTypeInterface interface;
} GunpowderBarrelObjDescriptorTypeCore;

struct GunpowderBarrelObjDescriptorType {
    GunpowderBarrelObjDescriptorTypeCore descriptor;
    u32 padding;
};

struct GunpowderBarrelObjDescriptorType gGunpowderBarrelObjDescriptor = {
    {
        {
            {
                0,
        ...
        {
            0,
            gGunpowderBarrelObjDescriptorInitAdapter,
            gunpowderBarrel_update,
            gunpowderBarrel_hitDetect,
            gunpowderBarrel_render,
            gunpowderBarrel_free,
            0,
            gGunpowderBarrelObjDescriptorExtraSizeAdapter,
            gunpowderBarrel_addThrowVelocity,
        },
    },
    0,
};
