...
void FlammableVine_update(GameObject* obj) {
    ...
    if (ObjAnim_ReadPlacementS16(&obj->anim, &(placement->gateBit)) == -1 ||
        (mainGetBit(ObjAnim_ReadPlacementS16(&obj->anim, &(placement->gateBit))) != 0 && tricky != NULL &&
    ...
    if ((state->flags & FLAMMABLEVINE_FLAG_BURNING) != 0) {
        ...
        objfx_spawnPulseBurst(obj, FLAMMABLEVINE_PARTICLE_SCALE * (state->burnIntensity * obj->anim.rootMotionScale), 3,
                              0, pulseStyle, NULL);
...
void FlammableVine_init(GameObject* obj, FlammableVinePlacement* placement) {
    ...
    obj->anim.rootMotionScale =
        FLAMMABLEVINE_DEFAULT_SCALE *
        ((f32)ObjAnim_ReadPlacementS16(&obj->anim, &(placement->scaleParam)) / FLAMMABLEVINE_SCALE_DIVISOR);
    ...
    if (ObjAnim_ReadPlacementS16(&obj->anim, &(placement->burnedBit)) != -1 &&
        mainGetBit(ObjAnim_ReadPlacementS16(&obj->anim, &(placement->burnedBit))) != 0) {
        Obj_RemoveFromUpdateList(obj);
        ObjHits_DisableObject(obj);
        obj->anim.alpha = 0;
        state->flags |= FLAMMABLEVINE_FLAG_CONSUMED;
