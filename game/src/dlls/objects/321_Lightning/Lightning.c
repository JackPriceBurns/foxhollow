...
void lightning_update(GameObject* obj) {
    ...
    if (state->effect == 0 && state->flags.enabled) {
        ...
        if (state->countdown <= 0.0f) {
            state->countdown += (f32)(s32)((u32)placement->repeatDelay * LIGHTNING_DELAY_FRAME_SCALE);
            spawnLightning = 1;
        }
        if (spawnLightning != 0) {
            ...
            if ((targetState->modeBits.mode & LIGHTNING_MODE_HIT_EFFECT) != 0) {
                objfx_spawnHitEffectBurst(target, targetState->hitRadius, LIGHTNING_HIT_EFFECT_ID,
                                          LIGHTNING_HIT_EFFECT_VARIANT, LIGHTNING_HIT_EFFECT_COUNT, NULL);
            }
            ...
            if ((targetState->modeBits.mode & LIGHTNING_MODE_DIRECTIONAL_BURST) != 0) {
                objfx_spawnDirectionalBurst(target, LIGHTNING_BURST_INDEX, targetState->burstRadius,
                                            LIGHTNING_BURST_KIND, LIGHTNING_BURST_MODE, LIGHTNING_BURST_CHANCE,
                                            LIGHTNING_BURST_MULTIPLIER, NULL, 0);
            }
        }
    }
