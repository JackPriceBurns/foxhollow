...
void SB_Propeller_update(GameObject* obj) {
    ...
    if (obj->anim.parent != NULL) {
        ...
        if (galleonPhase == 1 && ObjHits_GetPriorityHit(obj, &hitObjectAddress, 0, 0) != 0 && obj->userData1 == 0 &&
            ...
            hitObjectAddress->anim.romDefNo != SB_OTHER_SEQ_ID && (obj->userData1 = 0x14, obj->anim.parent != NULL) &&
            (galleonStage == 2 || galleonStage == 5) && obj->anim.romDefNo == SB_PROPELLER_SEQ_ID) {
...
void SB_Propeller_init(GameObject* obj, SBPropellerPlacementView* placement) {
    ...
    state->spinRate = 1200;
    state->health = 4;
    obj->anim.bankIndex = (s8)ObjAnim_ReadPlacementS16(&obj->anim, &placement->modelBankIndex);
    if (obj->anim.romDefNo != SB_PROPELLER_SEQ_ID) {
