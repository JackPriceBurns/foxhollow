    ...
    if (state->baddie.moveDone != '\0' || state->baddie.moveJustStartedB != 0) {
        ...
        for (; objectIndex < objectCount; objectIndex++) {
            ...
            if (sibling != obj && sibling->anim.romDefNo == DLL_CE_SIBLING_SEQ_ID) {
                int siblingState = (*(DllCESiblingInterface**)sibling->anim.dll)->getControlMode(sibling, 0);
...
int dll_CE_updateAlertState(GameObject* obj, GroundBaddieState* state) {
    ...
    if (state->baddie.moveJustStartedA != '\0') {
        ...
        for (; objectIndex < objectCount; objectIndex++) {
            ...
            if (sibling != obj && sibling->anim.romDefNo == DLL_CE_SIBLING_SEQ_ID) {
                (*(DllCESiblingInterface**)sibling->anim.dll)->handleMessage(sibling, DLL_CE_MESSAGE_RELEASE, 0);
            }
        }
        playerChild = ((GameObject*)Obj_GetPlayerObject())->childObjs[0];
        player = Obj_GetPlayerObject();
        childState = (*(DllCEStaffInterface**)playerChild->anim.dll)->getHitReactValue((GameObject*)playerChild);
        if (childState != 0) {
...
int dll_CE_updateSpitState(GameObject* obj, GroundBaddieState* state) {
    ...
    if ((s32)state->baddie.moveJustStartedA != 0) {
        ...
        while (objectIndex < objectCount) {
            ...
            if ((void*)siblingAddress != (void*)obj && siblingAddress->anim.romDefNo == DLL_CE_SIBLING_SEQ_ID) {
...
void dll_CE_updateTargeting(GameObject* obj, GroundBaddieState* objectStateAddress, GroundBaddieState* stateAddress) {
    ...
    if (target != NULL) {
        ...
        stateAddress->baddie.targetDistance = sqrtf(delta.z * delta.z + (delta.x * delta.x + delta.y * delta.y));
    }

    if ((objectStateAddress->configFlags & 0x20) == 0) {
        (*gBaddieControlInterface)
            ->pollCameraTarget(obj, stateAddress, &objectStateAddress->flags400, 2, 3, objectStateAddress->soundIdA,
                               objectStateAddress->soundIdB);
    }

    (*gBaddieControlInterface)
        ->processMessages(obj, stateAddress, &objectStateAddress->routeNav, objectStateAddress->gameBitB, NULL, 0, 0,
                          8);

    hitReactionUpdated =
        (*gBaddieControlInterface)
            ->updateHitReaction(obj, stateAddress, &objectStateAddress->routeNav, objectStateAddress->gameBitB,
                                gDllCEHitReactionMoves, gDllCEHitReactionDamage, 1, &gDllCEHitReactionScratch);
...
void dll_CE_update(GameObject* obj, int unusedA, int unusedB) {
    ...
    if (obj->userData1 != 0) {
        ...
        if (state->targetState == 0) {
            ...
            (*gPlayerInterface)->update(obj, state, timeDelta, timeDelta, gDllCEMoveHandlers, gDllCECheckHandlers);
