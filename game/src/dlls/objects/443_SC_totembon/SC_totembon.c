...
static const u16 sTotemBondRingGameBits[SC_TOTEM_BOND_RING_COUNT] = {
    GAMEBIT_SC_TotemBondRing0Active,
    GAMEBIT_SC_TotemBondRing1Active,
    GAMEBIT_SC_TotemBondRing2Active,
    GAMEBIT_SC_TotemBondRing3Active,
    GAMEBIT_SC_TotemBondRing4Active,
    GAMEBIT_SC_TotemBondRing5Active,
    GAMEBIT_SC_TotemBondRing6Active,
    GAMEBIT_SC_TotemBondRing7Active,
};

static const u16 sTotemBondOrbGameBits[SC_TOTEM_BOND_RING_COUNT] = {
    GAMEBIT_SC_LightfootChallengeDifficulty0,
    GAMEBIT_SC_LightfootChallengeDifficulty1,
    GAMEBIT_SC_LightfootChallengeDifficulty2,
    GAMEBIT_SC_LightfootChallengeDifficulty3,
    GAMEBIT_SC_LightfootChallengeDifficulty4,
    GAMEBIT_SC_LightfootChallengeDifficulty5,
    GAMEBIT_SC_LightfootChallengeDifficulty6,
    GAMEBIT_SC_LightfootChallengeDifficulty7,
};
...
static void sc_totembond_spawnGameBitOrbs(GameObject* obj, f32 radius) {
    ...
    for (int spawnIndex = 0; spawnIndex < SC_TOTEM_BOND_RING_COUNT; spawnIndex++) {
        ...
        placement->base.posX =
            radius * mathSinf((3.1415927f * (f32)(obj->anim.rotX + angleOffset)) / 32768.0f) +
            obj->anim.localPosX;
        placement->base.posY = obj->anim.localPosY;
        placement->base.posZ =
            radius * mathCosf((3.1415927f * (f32)(obj->anim.rotX + angleOffset)) / 32768.0f) +
            obj->anim.localPosZ;
...
static void sc_totembond_update(GameObject* obj) {
    ...
    if ((state->eventFlags & SC_TOTEM_BOND_SET_MAP_MODE) != 0) {
        (*gMapEventInterface)->setMapAct(SC_LEVEL_CONTROL_MAP_SWAPCIRCLE,
                                         SC_LEVEL_CONTROL_MAP_ACT_ESCAPE_COMPLETE);
        state->eventFlags &= ~SC_TOTEM_BOND_SET_MAP_MODE;
    }
}
...
int sc_totembond_insertOrderedGameBit(const u16 gameBitIds[3], u16 newValue) {
    ...
    for (int pass = 0; pass < 3; pass++) {
        for (int index = 0; index < 3; index++) {
            if (values[index + 1] != 0 &&
                (values[index + 1] < values[index] || values[index] == 0)) {
