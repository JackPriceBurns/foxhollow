...
void SeqPoint_init(GameObject* obj, WmSeqPointMapData* data)
{
    ...
    state->triggerRadius = ObjAnim_ReadPlacementS16(&obj->anim, &(def->triggerRadius));
    state->sequenceId = ObjAnim_ReadPlacementS16(&obj->anim, &(def->sequenceId));
    state->triggerMode = def->triggerMode;
    state->conditionGameBit = ObjAnim_ReadPlacementS16(&obj->anim, &(def->conditionGameBit));
    state->disableGameBit = ObjAnim_ReadPlacementS16(&obj->anim, &(def->disableGameBit));
