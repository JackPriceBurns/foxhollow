...
void kytesmum_update(GameObject* obj) {
    ...
    if ((s16)(runtime->idleSfxTimer -= framesThisStep) < 0) {
        ...
        objSoundStartFromDef(obj, &runtime->modelSoundState, &runtime->idleSfxTable[randomGetRange(0, 3)], 0);
    }
    if (ObjAnim_AdvanceCurrentMove(obj, runtime->animSpeed, timeDelta, (ObjAnimEventList*)runtime->animEvents) != 0) {
    ...
    if (nearest != NULL) {
        TRICKY_INTERFACE(nearest)->sideCommandEnable(nearest, obj, KYTESMUM_TRICKY_COMMAND_KIND,
