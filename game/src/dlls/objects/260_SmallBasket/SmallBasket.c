...
int SmallBasket_spawnContents(GameObject* obj, GameObject* player, SmallBasketState* state) {
    ...
    if (state->subtype == SMALLBASKET_SUBTYPE_RANDOM) {
        health = (f32)Player_GetCurrentHealth((uintptr_t)player);
        healthPercent = health;
        maxHealth = (f32)Player_GetMaxHealth((uintptr_t)player);
...
void SmallBasket_init(GameObject* obj, SmallBasketPlacement* placement) {
    ...
    respawnMinutes = ObjAnim_ReadPlacementS16(&obj->anim, &placement->respawnMinutes);
    if (respawnMinutes == 0) {
    ...
    state->unk1F = (u8)ObjAnim_ReadPlacementS16(&obj->anim, &placement->unk1A);
