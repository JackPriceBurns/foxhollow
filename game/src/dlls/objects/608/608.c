...
void ProximityMine_expire(GameObject* obj) {
    ...
    objfx_shakeCameraByDistance(obj, 200.0f);
    {
        ...
        spawnExplosion(obj, 60.0f + triggerRadiusDelta * gProximityMineExplosionRadiusScale, 1, 1, 0, 1, 0, 1, 0);
    }
    ...
    if (timerIsActive(&state->lifespanTimer) != 0) {
        ...
        if (state->effectHandle == NULL) {
            ...
            if (state->effectHandle != NULL) {
                ...
                modelLightStruct_setupGlow(state->effectHandle, 0, 0xff, 0, 0, gProximityMineGlowAlpha,
        ...
        if (timerIsActive(&state->resetTimer) != 0) {
            ...
            if (state->effectHandle == NULL) {
                state->effectHandle = modelLightStruct_createPointLight(obj, 0xff, 0, 0, 0);
                if (state->effectHandle != NULL) {
                    modelLightStruct_setupGlow(state->effectHandle, 0, 0xff, 0, 0, gProximityMineResetGlowAlpha,
