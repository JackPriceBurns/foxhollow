...
int LandedArwing_TriggerLaunchTarget(GameObject* obj, BaddieState* baddie) {
    ...
    if (baddie->moveJustStartedA != 0) {
        (*gBaddieControlInterface)->spawnChild(obj, (int)aux->triggerId, -1, 0);
...
int LandedArwing_UpdateBounceFade(GameObject* obj, BaddieState* baddie) {
    ...
    obj->anim.velocityX = obj->anim.velocityX * (horizontalDamping = 0.985f);
    obj->anim.velocityY = 0.945f * (obj->anim.velocityY - 0.4f);
...
u32 LandedArwing_UpdateFlightChase(GameObject* obj, BaddieState* state) {
    ...
    if (state->moveJustStartedA != 0) {
        ...
        obj->anim.velocityX = -sub->speed * fsin16Precise(obj->anim.rotX & 0xffff);
        obj->anim.velocityY = 0.0f;
        obj->anim.velocityZ = -sub->speed * fcos16Precise(obj->anim.rotX & 0xffff);
    ...
    if (sub->surfaceMode != LANDED_ARWING_SCRIPT_MODE) {
        if (playerObj != NULL && playerObj->anim.worldPosX >= sub->boundsMinX &&
            playerObj->anim.worldPosX <= sub->boundsMaxX && playerObj->anim.worldPosY >= sub->boundsMinY &&
            playerObj->anim.worldPosY <= sub->boundsMaxY && playerObj->anim.worldPosZ >= sub->boundsMinZ &&
...
void landedarwing_moveAlongSurface(GameObject* obj, LandedArwingState* state) {
    ...
    hitFound = trackGetIntersect(obj, start, end, 1, hitScratch.hit, 0x20);
    if (hitFound != 0) {
        if ((((hitScratch.hit[0] != state->surfaceNormalX) || (hitScratch.hit[1] != state->surfaceNormalY)) ||
...
void dll_D3_render(GameObject* obj, int p2, int p3, int p4, int p5, s8 visible) {
    ...
    if (visible != 0) {
        ...
        case 0:
            if ((state->surfaceMode == 6) && (state->flags92.surfaceOrientationReady != 0)) {
                if (state->flags92.airborne == 0) {
                    landedarwing_buildSurfaceOrientationMatrix(slideMtx, &(obj)->anim.velocityX,
                                                               &state->surfaceNormalX);
                }
...
#define dz           vec[3]
    ...
    if (state->targetState != 1) {
        GameObject* target =
            (*gBaddieControlInterface)->findAggroTarget(obj, state, (f32)(u32)state->aggroRange, 0x8000);
        if (target != NULL) {
            (*gBaddieControlInterface)
                ->startHitReaction(obj, state, &state->routeNav, state->gameBitB, NULL, 0, 1, 0, -1);
    ...
    if (state->baddie.targetObj != 0u) {
        dx = ((GameObject*)(state->baddie.targetObj))->anim.worldPosX - obj->anim.worldPosX;
        dy = ((GameObject*)(state->baddie.targetObj))->anim.worldPosY - obj->anim.worldPosY;
        dz = ((GameObject*)(state->baddie.targetObj))->anim.worldPosZ - obj->anim.worldPosZ;
        state->baddie.targetDistance = sqrtf(dz * dz + (dx * dx + dy * dy));
    }

    (*gBaddieControlInterface)->processMessages(obj, state, &state->routeNav, state->gameBitB, NULL, 0, 0, 0);

    hits = (int)state->baddie.hitPoints;
    if (hits > 0) {
        (*gBaddieControlInterface)
            ->updateHitReaction(obj, state, &state->routeNav, state->gameBitB, gStaffActionHitReactionMoves,
                                gStaffActionHitReactionDamage, 0, &gStaffActionHitLightParams);
    ...
    (*gPlayerInterface)
        ->update(obj, state, timeDelta, timeDelta, gLandedArwingStateHandlers, &gLandedArwingDefaultStateHandler);

    obj->pendingParentObj = state->savedPendingParentObj;

    if (extra->flags92.scriptTargetActive == 0u && extra->surfaceMode == 6) {
        hitCount = trackGetLineIntersect(&obj->anim.previousLocalPosX, &obj->anim.localPosX, 6.0f, 0, &hitResult, obj,
                                         -0x7c, -1, 0xff, 0);
...
void dll_D3_init(GameObject* obj, DllD3Placement* def, int flag) {
    ...
    (*gBaddieControlInterface)->initGroundBaddie(obj, (u8*)def, (u8*)state, 5, 1, 0x108, setupFlags, 20.0f);
