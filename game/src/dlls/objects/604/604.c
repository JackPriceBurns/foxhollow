...
const SnowClawDropObjectTable gSnowClawDropObjectTable = {
    {SNOWCLAW_DROP_OBJ_SWORD, SNOWCLAW_DROP_OBJ_STAFF, SNOWCLAW_DROP_OBJ_SCWEAPON_T1, SNOWCLAW_DROP_OBJ_ICEBALL, 0x1D}};
...
void snowclaw_spawnDropBomb(GameObject* obj, GameObject* owner, int launchMode, int userData1Value) {
    ...
    if (Obj_IsLoadingLocked() != 0) {
        ...
        setup->aimYaw = (s8)(u8)((((getAngle(player->anim.localPosX - obj->anim.localPosX,
        ...
        case 0:
            ...
            break;
        case 1:
            setup->launchAngle = (s16)(getAngle(player->anim.localPosX - obj->anim.localPosX,
                                                player->anim.localPosZ - obj->anim.localPosZ) +
                                       0x8000);
            break;
        }
...
void snowclaw_updateMountAttack(GameObject* obj, GameObject* mount) {
    ...
    if (mountFlag != 0 && (obj)->anim.currentMove == inner->moveIdBase) {
        Object_ObjAnimSetSecondaryBlendMove(&obj->anim, inner->moveIdBase + 1, magnitude);
    } else {
        Object_ObjAnimSetSecondaryBlendMove(&obj->anim, inner->moveIdBase + 2, magnitude);
    }
...
int snowclaw_animEventCallback(GameObject* obj, int a2, ObjSeqState* seq) {
    ...
    if (obj->seqIndex != -1 &&
        (obj->anim.romDefNo == SNOWCLAW_SEQID_IM_SNOWCLAW || obj->anim.romDefNo == SNOWCLAW_SEQID_IM_SNOWCLAW2) &&
    ...
    if (s->dropIndex != s->dropIndexApplied) {
        ...
        if (s->dropIndex > 0 && Obj_IsLoadingLocked() != 0) {
            obj->childObjs[0] = objSetupObject(Obj_AllocObjectSetup(0x18, tbl.objectIds[s->dropIndex]), 4,
...
void snowclaw_render(GameObject* obj, int p2, int p3, int p4, int p5, s8 vis) {
    ...
    if (vis != 0 && s->mountAlpha != 0) {
        ...
        if ((obj)->childCount == 0 && (obj)->anim.romDefNo == SNOWCLAW_SEQID_CR_SNOWCLAW && s->stateFlags.b80 != 0) {
        ...
        ObjPath_GetPointWorldPosition(obj, 1, &s->posX, &s->posY, &s->posZ, 0);
...
void snowclaw_hitDetect(GameObject* obj) {
    ...
    if (ObjHits_GetPriorityHit(sub, &hit, 0, 0) == 0x15 && s->health >= 0) {
        ...
        if (s->hitCooldown < 0) {
            ...
            if (s->health < 0) {
                ...
                if (obj->anim.romDefNo == SNOWCLAW_SEQID_IM_SNOWCLAW ||
                    obj->anim.romDefNo == SNOWCLAW_SEQID_IM_SNOWCLAW2) {
                ...
                s->velX = 0.1f * mathSinf(3.1415927f * (f32)obj->anim.rotX / 32768.0f);
                s->velY = 0.01f * (f32)randomGetRange(0x28, 0x64);
                s->velZ = 0.1f * mathCosf(3.1415927f * (f32)obj->anim.rotX / 32768.0f);
    ...
    if (s->mount != NULL && SNOWCLAW_MOUNT_INTERFACE(s->mount)->getRiderMode(s->mount) == 2) {
...
void snowclaw_update(GameObject* obj) {
    ...
    dropTable = gSnowClawDropObjectTable;
    if (s->dropIndex != s->dropIndexApplied) {
        if (obj->childObjs[0] != NULL) {
            Obj_FreeObject((GameObject*)obj->childObjs[0]);
            obj->childObjs[0] = NULL;
        ...
        if (s->dropIndex > 0 && Obj_IsLoadingLocked() != 0) {
            obj->childObjs[0] = objSetupObject(Obj_AllocObjectSetup(0x18, dropTable.objectIds[s->dropIndex]), 4,
                                               obj->anim.mapEventSlot, -1, obj->anim.parent);
            obj->childCount = 1;
        }
        s->dropIndexApplied = s->dropIndex;
    ...
    sub = s->mount;
    if (sub != 0 && s->health != 0 && obj->anim.currentMove == s->moveIdBase && SnowBike_isAtRankGate(sub) != 0 &&
        timerCountDown(&s->attackTimer) != 0) {
        ...
        s16toFloat(&s->attackTimer, gSnowClawAttackTimerByRank[SnowBike_getRouteRank(s->mount) - 1]);
    }
