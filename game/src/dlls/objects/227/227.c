...
void Fireball_update(GameObject* obj) {
    ...
    if (state->elapsedTime > state->flightDuration) {
        ObjHits_SetHitVolumeSlot((ObjAnimComponent*)obj, FIREBALL_HIT_VOLUME_SLOT,
                                 placement->hitVolumeMode != 0 ? 3 : 1, 0);
    }
...
void Fireball_init(GameObject* obj) {
    ...
    if (ObjAnim_ReadPlacementS16(&obj->anim, &(placement->startDisabled)) != 0) {
        ...
        if (state->light == NULL) {
            ...
            if (state->light != NULL) {
                ...
                c = state->colorIndex * 3;
                modelLightStruct_setupGlow(state->light, 0, ((u8*)gFireballLightColors)[c], base1[c], base2[c], 32,
                                           50.0f);
        ...
        if (obj->anim.romDefNo != FIREBALL_SEQID_HIDDEN &&
            ObjAnim_ReadPlacementS16(&obj->anim, &(placement->startupDelayEnabled)) != 0) {
