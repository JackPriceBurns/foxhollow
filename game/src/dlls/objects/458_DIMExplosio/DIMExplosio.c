        ...
        for (i = 0, cursor = (uintptr_t)state; i < state->flameCount; i++) {
            if (((DimExplosionFlame*)cursor)->active != 0) {
                ...
                explosion_computeColor((f32)((DimExplosionFlame*)cursor)->age,
                                       (f32)((DimExplosionFlame*)cursor)->lifetime, state->modelKind, (u8*)&colA);
        ...
        if (((DimExplosionFlame*)cursor)->active != 0) {
            ...
            *(s8*)&((DimExplosionFlame*)cursor)->alpha = sExplosionColorMax[0] - gExplosionDebrisAlphaScale * ev;
            if (((DimExplosionFlame*)cursor)->age >= ((DimExplosionFlame*)cursor)->lifetime) {
                ...
                ((DimExplosionFlame*)cursor)->spinAngle += framesThisStep * ((DimExplosionFlame*)cursor)->spinSpeed;
...
void explosion_init(GameObject* obj, DimExplosionPlacement* placementAddress) {
    ...
    if (ObjAnim_ReadPlacementS16(&obj->anim, &(placementAddress->scaleParam)) == 0) {
        ...
        scale =
            (f32)(int)ObjAnim_ReadPlacementS16(&obj->anim, &(placementAddress->scaleParam)) * sExplosionSpeedScale[0];
        if (scale > 100.0f) {
            scale = 100.0f;
        }
    }
