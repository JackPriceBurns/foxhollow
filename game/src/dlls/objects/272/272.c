...
int Door_animEventCallback(GameObject* obj, int unused, ObjSeqState* animUpdate) {
    ...
    if (state->phase == DOOR_PHASE_OPEN) {
        ...
        closeReady = 0;
        if (ObjAnim_ReadPlacementS16(&obj->anim, &(placement->closeReadyGameBit)) == DOOR_NO_GAME_BIT ||
            mainGetBit(ObjAnim_ReadPlacementS16(&obj->anim, &(placement->closeReadyGameBit))) != 0) {
            closeReady = 1;
        }
