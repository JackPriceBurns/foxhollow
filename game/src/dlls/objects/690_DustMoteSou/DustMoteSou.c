...
void dustmotesou_update(GameObject* source) {
    ...
    if (source->anim.romDefNo == DUST_MOTE_SOURCE_TAIL_LIGHT) {
        ...
        objfx_spawnMaskedHitEffect(source, scale, placement->effectId, placement->effectParamA, placement->effectParamB,
                                   NULL);
        return;
    }
    if (source->anim.romDefNo == DUST_MOTE_SOURCE_FIREWORK) {
        ...
        objfx_spawnHitEffectBurst(source, scale, placement->effectId, placement->effectParamA, placement->effectParamB,
                                  NULL);
        return;
    }
    ...
    if (placement->burstMode == DUST_MOTE_BURST_BOX) {
        objfx_spawnBoxBurst(source, placement->effectId, scale, placement->effectParamA, placement->effectParamB,
                            placement->spawnChance, placement->spreadX, placement->spreadY, placement->spreadZ, NULL,
                            0);
    } else if (placement->burstMode == DUST_MOTE_BURST_ARCED) {
        objfx_spawnArcedBurst(source, placement->effectId, scale, placement->effectParamA, placement->effectParamB,
                              placement->spawnChance, placement->spreadX, placement->spreadY, placement->spreadZ, NULL,
                              0);
    } else {
