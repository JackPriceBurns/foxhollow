...
void mclightning_render(GameObject* obj, int p2, int p3, int p4, int p5, f32 scale) {
    ...
    if (mode == MCLIGHTNING_PHASE_ARMED) {
        ...
        if (i == count) {
            ...
            state->boltHandle =
                lightningCreate((const Vec3f*)&obj->anim.localPosX, (const Vec3f*)&target->anim.localPosX,
                                state->boltParamA, state->boltParamB, state->boltParamC, state->boltParamD, 0);
            ...
            foundState = target->extra;
            if (foundState->flags.spawnFlags & 1) {
                objfx_spawnHitEffectBurst(target, foundState->hitEffectScale, 1, 7, 0x1e, NULL);
            }
            ...
            if (foundState->flags.spawnFlags & 2) {
                objfx_spawnDirectionalBurst(target, 5, foundState->burstEffectChance, 1, 1, 0x64, 5.0f, NULL, 0);
            }
        }
