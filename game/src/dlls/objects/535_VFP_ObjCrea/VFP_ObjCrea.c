...
void VFP_ObjCreator_update(GameObject* obj) {
    ...
    case VFP_OBJCREATOR_FALLING_MODE:
        ...
        if (state->spawnTimer <= 0) {
            ...
            setupBuf->base.posX = obj->anim.localPosX + (f32)randomGetRange(-state->spawnRadius, state->spawnRadius);
            setupBuf->base.posY = obj->anim.localPosY;
            setupBuf->base.posZ = obj->anim.localPosZ + (f32)randomGetRange(-state->spawnRadius, state->spawnRadius);
            setupBuf->unk20 = 0x50;
            ...
            setupBuf->unk24 = 0;
            spawned = objSetupObject(&setupBuf->base, 5, obj->anim.mapEventSlot, -1, obj->anim.parent);
            if (spawned == NULL) {
                break;
            }
