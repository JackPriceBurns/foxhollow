...
void dll_1DA_update(GameObject* obj) {
    ...
    hitCount = trackGetLineIntersect(&obj->anim.previousLocalPosX, &obj->anim.localPosX, 6.5f, 1, &collision, obj, 8,
                                     -1, 0xff, 0);
    if (hitCount != 0) {
    ...
    hitCount = trackGetHeight(obj, obj->anim.localPosX, obj->anim.localPosY, obj->anim.localPosZ, &groundHits, 0, 0x11);
    state->grounded = 0;
    groundHitIndex = 0;
