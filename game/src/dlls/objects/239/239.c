...
void pushable_init(GameObject* obj, PushableObjectDef* setup) {
    ...
    state->scale = (f32)ObjAnim_ReadPlacementU16(&obj->anim, &setup->scaleRaw) / PUSHABLE_SCALE_DENOM;
    state->scale = state->scale * obj->anim.modelInstance->rootMotionScaleBase;
    state->cullDistance = state->scale * (f32)modelFileHeaderGetCullDistance(model) + PUSHABLE_MIN_GROUND_CLEARANCE;
    {
