...
typedef struct BaddieState {
    ...
    union {
        ...
        struct {
            ...
            u8 unkBD[0xC4 - 0xBD];
            u8 sphereMapFadeFlags;
            u8 padC5[0x118 - 0xC5 + 12];
...
typedef struct GroundBaddieState {
    ...
    CharacterEyeAnimState
        eyeAnimState; /* 0x3ac: head-aim / eye-blink record (characterDoEyeAnims / characterSetHeadYawToTarget) */
...
#define BADDIE_FLAG400_PATH_ACTIVE 0x8
    ...
    u8 unk40B;
    void*
        control; /* per-family control/extra record (engine-allocated; treasurechest casts its slot to LandedArwingState*) */
} GroundBaddieState;
