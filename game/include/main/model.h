...
typedef struct ModelFileHeader {
    ...
    u8* unk18;
    u8* unk1C;
    uintptr_t* textureIds; /* file texture ids, patched to full Texture pointers on load */
...
typedef struct ObjModel {
    ...
    u8**
        blendAnimData; /* 0x44: per-entry pointer array (file->blendAnimCount), filled from normalBuf + blendAnimEntries[i]+0x60 */
