...
static void musyxDecodeFrame(MusyxMixVoice* state, const DSPvoice* voice, u32 frameIndex) {
    ...
    for (i = 0; i < 14; i++) {
        ...
        sample =
            (((nibble << shift) << 11) + 1024 + coefficient1 * state->history1 + coefficient2 * state->history2) >> 11;
...
static void musyxDecodeSample(MusyxMixVoice* state, const DSPvoice* voice) {
    ...
    if ((voice->smp_info.compType != SAMPLE_TYPE_ADPCM && voice->smp_info.compType != SAMPLE_TYPE_ADPCM_PLUS) ||
        voice->smp_info.length == 0 || voice->smp_info.extraData == NULL || ARGetStorageAddress() == NULL) {
        return;
    }
...
static u32 musyxOffsetSampleIndex(const MusyxMixVoice* state, const DSPvoice* voice, u32 sampleIndex, s32 offset) {
...
static u32 musyxVoiceMatches(const MusyxMixVoice* state, const DSPvoice* voice) {
    return state->active != 0 && state->compType == voice->smp_info.compType && state->sampleId == voice->smp_id &&
           state->callbackId == voice->mesgCallBackUserValue && state->aramBase == (u32)(uintptr_t)voice->smp_info.addr;
}
    ...
    for (voiceIndex = 0; voiceIndex < salNumVoices && voiceIndex < MUSYX_MIX_VOICES; voiceIndex++) {
        ...
        for (frame = 0; frame < MUSYX_MIX_FRAMES; frame++) {
            ...
            if (fraction != 0 && voice->srcTypeSelect == 0) {
                ...
                s16 nextSample = musyxReadSample(state, voice, musyxOffsetSampleIndex(state, voice, sampleIndex, 1));
                sample = musyxClampSample((s32)sample + ((((s32)nextSample - sample) * (s32)fraction) >> 16));
            }
            if (envelope < 0) {
                envelope = 0;
