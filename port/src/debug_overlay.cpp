...
void sampleTimingFrame(Uint64 now) {
  ...
  if (sHaveMarkerSample && sPreviousFadeIn != 0 && fadeIn == 0 && alpha > 0 && press != nullptr &&
      !press->haveStop) {
    press->haveStop = true;
...
void drawTimingWindow(Uint64 now) {
  ...
  const float speed = (current != nullptr && previous != nullptr)
                          ? static_cast<float>(current->marker - previous->marker)
                          : 0.0f;
